...
import { resolveStateDir, type NsemclawConfig } from "../config/config.js";
...
function collectAuthStorePaths(config: NsemclawConfig, stateDir: string): string[] {
...
export async function runSecretsAudit(
  ...
  const previousAuthStoreReadOnly = process.env.NSEMCLAW_AUTH_STORE_READONLY;
  process.env.NSEMCLAW_AUTH_STORE_READONLY = "1";
  try {
    ...
    const config = snapshot.valid ? snapshot.config : ({} as NsemclawConfig);
    ...
    if (previousAuthStoreReadOnly === undefined) {
      delete process.env.NSEMCLAW_AUTH_STORE_READONLY;
    } else {
      process.env.NSEMCLAW_AUTH_STORE_READONLY = previousAuthStoreReadOnly;
    }
  }
}
