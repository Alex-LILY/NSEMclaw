...
function getSecretProviders(config: NsemclawConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: NsemclawConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function buildCandidates(config: NsemclawConfig): ConfigureCandidate[] {
...
function toSourceChoices(config: NsemclawConfig): Array<{ value: SecretRefSource; label: string }> {
...
async function configureProvidersInteractive(config: NsemclawConfig): Promise<void> {
  while (true) {
...
function collectProviderPlanChanges(params: { original: NsemclawConfig; next: NsemclawConfig }): {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    if (candidates.length === 0) {
      throw new Error("No configurable secret-bearing fields found in nsemclaw.json.");
    }
  ...
  const plan: SecretsApplyPlan = {
    ...
    generatedBy: "nsemclaw secrets configure",
