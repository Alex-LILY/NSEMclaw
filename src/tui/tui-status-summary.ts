...
export function formatStatusSummary(summary: GatewayStatusSummary) {
  const lines: string[] = [];
  lines.push(t("overview.snapshot.status"));

  if (!summary.linkChannel) {
    lines.push(`${t("overview.access.subtitle")}: ${t("common.unknown")}`);
  } else {
    ...
    const authAge =
      linked && typeof summary.linkChannel.authAgeMs === "number"
        ...
        : "";
    lines.push(`${linkLabel}: ${linked ? t("channels.linked") : t("common.offline")}${authAge}`);
  }

  const providerSummary = Array.isArray(summary.providerSummary) ? summary.providerSummary : [];
  if (providerSummary.length > 0) {
    lines.push("");
    lines.push(`${t("common.system")}:`);
  ...
  if (heartbeatAgents.length > 0) {
    ...
    lines.push("");
    lines.push(`${t("overview.snapshot.lastHeartbeat")}: ${heartbeatParts.join(", ")}`);
  }

  const sessionPaths = summary.sessions?.paths ?? [];
  if (sessionPaths.length === 1) {
    lines.push(`${t("sessions.title")}: ${sessionPaths[0]}`);
  } else if (sessionPaths.length > 1) {
    lines.push(`${t("sessions.title")}: ${sessionPaths.length}`);
  }
  ...
  const defaultCtx =
    typeof defaults?.contextTokens === "number"
      ...
      : "";
  lines.push(`${t("agents.primaryModel")}: ${defaultModel}${defaultCtx}`);

  const sessionCount = summary.sessions?.count ?? 0;
  lines.push(`${t("sessions.title")}: ${sessionCount}`);

  const recent = Array.isArray(summary.sessions?.recent) ? summary.sessions?.recent : [];
  if (recent.length > 0) {
    lines.push(`${t("sessions.title")}:`);
    for (const entry of recent) {
      const ageLabel = typeof entry.age === "number" ? formatTimeAgo(entry.age) : t("common.none");
