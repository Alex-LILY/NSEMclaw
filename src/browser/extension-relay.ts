...
import {
  probeAuthenticatedNsemclawRelay,
...
const RELAY_AUTH_HEADER = "x-nsemclaw-relay-token";
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const extensionReconnectGraceMs = envMsOrDefault(
    "NSEMCLAW_EXTENSION_RELAY_RECONNECT_GRACE_MS",
    DEFAULT_EXTENSION_RECONNECT_GRACE_MS,
  );
  const extensionCommandReconnectWaitMs = envMsOrDefault(
    "NSEMCLAW_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS",
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/Nsemclaw-Extension-Relay",
            revision: "0",
            userAgent: "Nsemclaw-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "Nsemclaw/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedNsemclawRelay({
