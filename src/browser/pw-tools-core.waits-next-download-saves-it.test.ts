...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredNsemclawTmpDir: vi.fn(() => "/tmp/openclaw"),
}));
vi.mock("../infra/tmp-nsemclaw-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredNsemclawTmpDir.mockReturnValue("/tmp/openclaw");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredNsemclawTmpDir.mockReturnValue("/tmp/openclaw-preferred");
    ...
    expect(tmpDirMocks.resolvePreferredNsemclawTmpDir).toHaveBeenCalled();
  });
