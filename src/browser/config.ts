import type { BrowserConfig, BrowserProfileConfig, NsemclawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_NSEMCLAW_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "nsemclaw" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_NSEMCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "nsemclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_NSEMCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_NSEMCLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Nsemclaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for openclaw/nsemclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: NsemclawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_NSEMCLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "nsemclaw";
