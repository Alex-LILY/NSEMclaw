...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".nsemclaw-profile-decorated");
}
 ...
 */
export function decorateNsemclawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_NSEMCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_NSEMCLAW_BROWSER_COLOR).toUpperCase();
