...
import { __testing, loadNsemclawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `nsemclaw-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.NSEMCLAW_BUNDLED_PLUGINS_DIR;
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.NSEMCLAW_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadNsemclawPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadNsemclawPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.NSEMCLAW_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.NSEMCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadNsemclawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadNsemclawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadNsemclawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.NSEMCLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ NSEMCLAW_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
