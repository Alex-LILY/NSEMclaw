...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `nsemclaw-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.nsemclaw/extensions and uses unscoped id", async () => {
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.nsemclaw).toBeUndefined();
