...
import { discoverNsemclawPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: NsemclawPluginDefinition;
  register?: NsemclawPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as NsemclawPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as NsemclawPluginDefinition;
...
export function loadNsemclawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverNsemclawPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "nsemclaw/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "nsemclaw/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: NsemclawPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as NsemclawPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
