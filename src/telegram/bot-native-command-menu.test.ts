...
describe("bot-native-command-menu", () => {
  ...
  it("fails after max retries on persistent network errors", async () => {
    const deleteMyCommands = vi.fn(async () => undefined);
    const setMyCommands = vi.fn().mockRejectedValue(new Error("Network request for 'setMyCommands' failed!"));
    ...
    // Wait for retries: 3 attempts with exponential backoff (500ms + 1000ms delays)
    await vi.waitFor(
      () => {
