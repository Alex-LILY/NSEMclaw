...
const SET_COMMANDS_MAX_RETRIES = 3;
const SET_COMMANDS_RETRY_DELAY_MS = 500;

async function withNetworkRetry<T>(fn: () => Promise<T>, maxRetries = 3, delayMs = 500): Promise<T> {
  let lastErr: unknown;
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      lastErr = err;
      if (!isRecoverableTelegramNetworkError(err) || i === maxRetries - 1) {
        throw err;
      }
...
export function syncTelegramMenuCommands(params: {
  ...
  const sync = async () => {
    ...
    if (typeof bot.api.deleteMyCommands === "function") {
      await withTelegramApiErrorLogging({
        ...
        runtime,
        fn: () => withNetworkRetry(() => bot.api.deleteMyCommands()),
        shouldLog: (err) => !isRecoverableTelegramNetworkError(err),
      }).catch(() => {});
    }
    ...
    while (retryCommands.length > 0) {
      try {
        await withTelegramApiErrorLogging({
          ...
          runtime,
          fn: () => withNetworkRetry(() => bot.api.setMyCommands(retryCommands), SET_COMMANDS_MAX_RETRIES, SET_COMMANDS_RETRY_DELAY_MS),
          shouldLog: (err) => !isRecoverableTelegramNetworkError(err),
        });
        return;
      } catch (err) {
  ...
  void sync().catch((err) => {
    // 网络错误降级为警告，非致命错误
    const isNetwork = isRecoverableTelegramNetworkError(err);
    const msg = `Telegram command sync failed: ${String(err)}`;
    if (isNetwork) {
      runtime.warn?.(`${msg} (network error, bot will continue working)`);
    } else {
      runtime.error?.(msg);
    }
  });
