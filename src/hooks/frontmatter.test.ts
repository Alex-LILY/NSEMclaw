...
`;
    ...
    expect(parsed.nsemclaw.emoji).toBe("💾");
    expect(parsed.nsemclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.nsemclaw.emoji).toBe("📝");
    expect(parsed.nsemclaw.events).toEqual(["command"]);
    expect(parsed.nsemclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.nsemclaw.install[0].kind).toBe("bundled");
  });
...
describe("resolveNsemclawMetadata", () => {
  it("extracts openclaw metadata from parsed frontmatter", () => {
    ...
    const result = resolveNsemclawMetadata(frontmatter);
...
`;
    ...
    const openclaw = resolveNsemclawMetadata(frontmatter);
