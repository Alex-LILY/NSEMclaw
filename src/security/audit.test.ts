...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: NsemclawConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: NsemclawConfig) => string[];
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync(
      { NSEMCLAW_STATE_DIR: channelSecurityStateDir },
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.NSEMCLAW_GATEWAY_TOKEN;
    const prevPassword = process.env.NSEMCLAW_GATEWAY_PASSWORD;
    delete process.env.NSEMCLAW_GATEWAY_TOKEN;
    delete process.env.NSEMCLAW_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.NSEMCLAW_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.NSEMCLAW_GATEWAY_PASSWORD;
      } else {
        process.env.NSEMCLAW_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    ...
    const configPath = path.join(stateDir, "nsemclaw.json");
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-nsemclaw.json");
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): NsemclawConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: NsemclawConfig = { session: { dmScope: "main" } };
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain(
        "~/.nsemclaw/credentials/discord-allowFrom.json:team.owner",
      );
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: NsemclawConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.NSEMCLAW_GATEWAY_TOKEN;
    process.env.NSEMCLAW_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: NsemclawConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<NsemclawConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: NsemclawConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.nsemclaw",
      configPath: "/Users/test/Dropbox/.nsemclaw/nsemclaw.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: NsemclawConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "nsemclaw.json"),
      });
