...
function cfg(accounts?: Record<string, unknown> | null): NsemclawConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as NsemclawConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as NsemclawConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as NsemclawConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as NsemclawConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as NsemclawConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as NsemclawConfig)).toBe("default");
    });
  });
});
