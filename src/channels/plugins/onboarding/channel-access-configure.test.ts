...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: NsemclawConfig, policy: ChannelAccessPolicy) => NsemclawConfig;
  resolveAllowlist: (params: { cfg: NsemclawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: NsemclawConfig; resolved: TResolved }) => NsemclawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: NsemclawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: NsemclawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: NsemclawConfig, policy: ChannelAccessPolicy): NsemclawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: NsemclawConfig, policy: ChannelAccessPolicy): NsemclawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: NsemclawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: NsemclawConfig; resolved: string[] }) => {
      calls.push("apply");
