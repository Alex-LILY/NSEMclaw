...
describe("promptParsedAllowFromForScopedChannel", () => {
  ...
  it("uses parser validation from the prompt validate callback", async () => {
    ...
    expect(next.channels?.signal?.allowFrom).toEqual(["ok"]);
  });
});
...
describe("setAccountAllowFromForChannel", () => {
  it("writes allowFrom on default account channel config", () => {
    ...
    expect(next.channels?.signal?.allowFrom).toEqual(["new-default"]);
    expect(next.channels?.signal?.accounts?.work?.allowFrom).toEqual(["work-old"]);
  });
...
describe("patchChannelConfigForAccount", () => {
  ...
  it("supports signal account-scoped channel patches", () => {
    const cfg: NsemclawConfig = {
      channels: {
        signal: {
          enabled: false,
