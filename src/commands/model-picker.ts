...
type PromptDefaultModelResult = { model?: string; config?: NsemclawConfig };
...
function resolveConfiguredModelRaw(cfg: NsemclawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: NsemclawConfig): string[] {
...
export function applyPrimaryModel(cfg: NsemclawConfig, model: string): NsemclawConfig {
...
export function applyModelAllowlist(cfg: NsemclawConfig, models: string[]): NsemclawConfig {
  const defaults = cfg.agents?.defaults;
