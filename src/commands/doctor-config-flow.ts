...
function stripUnknownConfigKeys(config: NsemclawConfig): {
  config: NsemclawConfig;
  removed: string[];
} {
  const parsed = NsemclawSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: NsemclawConfig) {
...
export function collectMissingDefaultAccountBindingWarnings(cfg: NsemclawConfig): string[] {
...
function scanTelegramAllowFromUsernameEntries(cfg: NsemclawConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: NsemclawConfig): Promise<{
  config: NsemclawConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: NsemclawConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: NsemclawConfig): {
  config: NsemclawConfig;
  changes: string[];
} {
...
function scanMutableAllowlistEntries(cfg: NsemclawConfig): MutableAllowlistHit[] {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: NsemclawConfig): {
  config: NsemclawConfig;
  changes: string[];
} {
...
async function maybeRepairAllowlistPolicyAllowFrom(cfg: NsemclawConfig): Promise<{
  config: NsemclawConfig;
  changes: string[];
}> {
  const channels = cfg.channels;
 ...
 * file support.
 */
function detectEmptyAllowlistPolicy(cfg: NsemclawConfig): string[] {
  const channels = cfg.channels;
  if (!channels || typeof channels !== "object") {
    return [];
  }
  ...
  const checkAccount = (
    ...
    if (dmPolicy === "allowlist" && !hasAllowFromEntries(effectiveAllowFrom)) {
      warnings.push(
        `- ${prefix}.dmPolicy is "allowlist" but allowFrom is empty — all DMs will be blocked. Add sender IDs to ${prefix}.allowFrom, or run "${formatCliCommand("nsemclaw doctor --fix")}" to auto-migrate from pairing store when entries exist.`,
      );
    }
...
function collectExecSafeBinScopes(cfg: NsemclawConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: NsemclawConfig): ExecSafeBinCoverageHit[] {
...
function scanExecSafeBinTrustedDirHints(cfg: NsemclawConfig): ExecSafeBinTrustedDirHintHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: NsemclawConfig): {
  config: NsemclawConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: NsemclawConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: NsemclawConfig): {
  config: NsemclawConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".nsemclaw");
  const targetPath = path.join(targetDir, "nsemclaw.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: NsemclawConfig = baseCfg;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("nsemclaw doctor --fix")}" to apply compatibility migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("nsemclaw doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (shouldRepair) {
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("nsemclaw doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "瑶姬警示",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("nsemclaw doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "瑶姬警示",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("nsemclaw doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "瑶姬警示",
      );
    }
    ...
    if (toolsBySenderHits.length > 0) {
      ...
      note(
        [
          ...
          `- Run "${formatCliCommand("nsemclaw doctor --fix")}" to migrate legacy keys to typed id: entries.`,
        ].join("\n"),
        "瑶姬警示",
      );
    }
    ...
    if (safeBinCoverage.length > 0) {
      ...
      lines.push(
        `- Run "${formatCliCommand("nsemclaw doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`,
      );
      note(lines.join("\n"), "瑶姬警示");
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "nsemclaw doctor --fix" to remove these keys.');
    }
  }
