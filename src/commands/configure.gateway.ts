...
export async function promptGatewayConfig(
  ...
  let authMode = guardCancel(
    await select({
      ...
      options: [
        { value: "token", label: "令牌", hint: "Recommended default" },
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Nsemclaw trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.nsemclaw.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
