...
function captureEnv(): EnvSnapshot {
  return {
    ...
    OPENCLAW_HOME: process.env.NSEMCLAW_HOME,
    NSEMCLAW_STATE_DIR: process.env.NSEMCLAW_STATE_DIR,
    OPENCLAW_OAUTH_DIR: process.env.NSEMCLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: NsemclawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: NsemclawConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    process.env.NSEMCLAW_HOME = tempHome;
    process.env.NSEMCLAW_STATE_DIR = path.join(tempHome, ".nsemclaw");
    delete process.env.NSEMCLAW_OAUTH_DIR;
    fs.mkdirSync(process.env.NSEMCLAW_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when OPENCLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.NSEMCLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: NsemclawConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
