...
export async function setupAuthTestEnv(
  ...
  const agentDir = path.join(stateDir, options?.agentSubdir ?? "智能体");
...
export function requireNsemclawAgentDir(): string {
  const agentDir = process.env.NSEMCLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("NSEMCLAW_AGENT_DIR not set");
  }
  return agentDir;
}
