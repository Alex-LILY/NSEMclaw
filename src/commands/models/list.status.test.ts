...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveNsemclawAgentDir: vi.fn().mockReturnValue("/tmp/openclaw-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveNsemclawAgentDir: mocks.resolveNsemclawAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveNsemclawAgentDir).toHaveBeenCalled();
