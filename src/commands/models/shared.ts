...
export async function loadValidConfigOrThrow(): Promise<NsemclawConfig> {
...
export async function updateConfig(
  mutator: (cfg: NsemclawConfig) => NsemclawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: NsemclawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: NsemclawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("nsemclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
