...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveNsemclawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("nsemclaw models auth setup-token")}\` or \`${formatCliCommand("nsemclaw configure")}\`.`
          : `Run \`${formatCliCommand("nsemclaw configure")}\` or set an API key env var.`;
