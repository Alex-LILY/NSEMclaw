...
const SANDBOX_DOCS_URL = "https://docs.nsemclaw.ai/sandbox";
...
export async function sandboxExplainCommand(
  ...
  if (!elevatedGlobalEnabled) {
    elevatedFailures.push({ gate: "已启用", key: "tools.elevated.enabled" });
  }
  ...
  lines.push(
    `  ${key("runtime:")} ${payload.sandbox.sessionIsSandboxed ? warn("sandboxed") : ok("私信")}`,
  );
  lines.push(
  ...
  if (payload.sandbox.mode === "non-main" && payload.sandbox.sessionIsSandboxed) {
    lines.push("");
    lines.push(
      `${warn("提示:")} sandbox mode is non-main; use main session key to run direct: ${value(
  ...
  lines.push("");
  lines.push(`${key("Docs:")} ${formatDocsLink("/sandbox", "docs.nsemclaw.ai/sandbox")}`);
