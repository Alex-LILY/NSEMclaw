...
export function normalizeCompatibilityConfigValues(cfg: NsemclawConfig): {
  config: NsemclawConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: NsemclawConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as NsemclawConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as NsemclawConfig["browser"],
    };
    changes.push(
  ...
  if (legacyAckReaction && hasWhatsAppConfig) {
    ...
    if (!hasWhatsAppAck) {
      ...
      let direct = true;
      let group: "总是" | "mentions" | "从未" = "mentions";
      if (legacyScope === "all") {
        direct = true;
        group = "总是";
