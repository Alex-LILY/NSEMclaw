...
const resolveAuthAgentDir = (agentDir?: string) => agentDir ?? resolveNsemclawAgentDir();
...
function resolveSiblingAgentDirs(primaryAgentDir: string): string[] {
  ...
  const looksLikeStandardLayout =
    path.basename(normalized) === "智能体" && path.basename(candidateAgentsRoot) === "agents";
  ...
  const discovered = entries
    ...
    .map((entry) => path.join(agentsRoot, entry.name, "智能体"));
...
export async function writeOAuthCredentials(
  ...
  const email = typeof creds.email === "string" && creds.email.trim() ? creds.email.trim() : "默认";
