...
export type ResetScope = "配置" | "config+creds+sessions" | "full";
...
export async function resetCommand(runtime: RuntimeEnv, opts: ResetOptions) {
  ...
  if (!["配置", "config+creds+sessions", "full"].includes(scope)) {
    runtime.error('Invalid --scope. Expected "配置", "config+creds+sessions", or "full".');
    runtime.exit(1);
    return;
  }
  ...
  if (scope === "config+creds+sessions") {
    ...
    runtime.log(`Next: ${formatCliCommand("nsemclaw onboard --install-daemon")}`);
    return;
  }
