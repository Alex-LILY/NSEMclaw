...
describe("writeOAuthCredentials", () => {
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    ...
    process.env.NSEMCLAW_AGENT_DIR = kidAgentDir;
  ...
  it("syncs siblings from explicit agentDir outside NSEMCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-oauth-external-"));
    process.env.NSEMCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* NSEMCLAW_STATE_DIR
