 ...
 * Runs as part of `nsemclaw doctor` — config-only, no network calls.
 */
export async function noteMemorySearchHealth(
  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          'Memory search provider is set to "本地" but no local model file was found.',
          "",
          "Fix (pick one):",
          `- Install node-llama-cpp and set a local model path in config`,
          `- Switch to a remote provider: ${formatCliCommand("nsemclaw config set agents.defaults.memorySearch.provider openai")}`,
          "",
          `Verify: ${formatCliCommand("nsemclaw memory status --deep")}`,
        ].join("\n"),
        "Memory search",
      );
      return;
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("nsemclaw configure --section model")}`,
        `- To disable: ${formatCliCommand("nsemclaw config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("nsemclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
