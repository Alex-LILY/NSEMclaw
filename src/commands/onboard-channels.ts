...
async function collectChannelStatus(params: {
  ...
  const fallbackStatuses = listChatChannels()
    ...
    .map((meta) => {
      ...
      const statusLabel = configured ? "configured (plugin disabled)" : "未配置";
      return {
        channel: meta.id,
        configured,
        statusLines: [`${meta.label}: ${statusLabel}`],
        selectionHint: configured ? "configured · plugin disabled" : "未配置",
        quickstartScore: 0,
      };
    });
...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `Approve with: ${formatCliCommand("nsemclaw pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelOnboardingDmPolicy) => {
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`nsemclaw pairing approve ${policy.channel} <code>`)}`,
    ...
    return (await prompter.select({
      ...
      options: [
        ...
        { value: "已禁用", label: "Disabled (ignore DMs)" },
      ],
    })) as DmPolicy;
  };
    ...
    if (!getChannelPlugin(channel)) {
      ...
      if (adapter) {
        await prompter.note(
          ...
          )}\` and \`${formatCliCommand("nsemclaw plugins enable " + channel)}\`, then restart the gateway.`,
          "Channel setup",
        );
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          value: "__skip__",
          label: "Skip for now",
          hint: `You can add channels later via \`${formatCliCommand("nsemclaw channels add")}\``,
        },
      ],
      ...
      const choice = (await prompter.select({
        ...
        options: [
          ...buildSelectionOptions(entries),
          {
            value: doneValue,
            label: "Finished",
            hint: selection.length > 0 ? "完成" : "Skip for now",
          },
        ],
        initialValue,
