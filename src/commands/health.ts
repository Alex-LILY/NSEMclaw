...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.NSEMCLAW_DEBUG_HEALTH)) {
...
const formatAccountProbeTiming = (summary: ChannelAccountHealthSummary): string | null => {
  ...
  const accountId = summary.accountId || "默认";
...
export const formatHealthChannelLines = (
  ...
  const accountMode = opts.accountMode ?? "默认";
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: NsemclawConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.NSEMCLAW_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
    ...
    const channelLines =
      Object.keys(accountIdsByChannel).length > 0
        ? formatHealthChannelLines(summary, {
            accountMode: opts.verbose ? "all" : "默认",
            accountIdsByChannel,
          })
        : formatHealthChannelLines(summary, {
            accountMode: opts.verbose ? "all" : "默认",
