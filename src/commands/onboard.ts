...
const VALID_RESET_SCOPES = new Set<ResetScope>(["配置", "config+creds+sessions", "full"]);
...
export async function onboardCommand(opts: OnboardOptions, runtime: RuntimeEnv = defaultRuntime) {
  ...
  if (normalizedOpts.resetScope && !VALID_RESET_SCOPES.has(normalizedOpts.resetScope)) {
    runtime.error('Invalid --reset-scope. Use "配置", "config+creds+sessions", or "full".');
    runtime.exit(1);
    return;
  }
  ...
  if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
    runtime.error(
      [
        ...
        "Read: https://docs.nsemclaw.ai/security",
        `Re-run with: ${formatCliCommand("nsemclaw onboard --non-interactive --accept-risk ...")}`,
      ].join("\n"),
    );
    runtime.exit(1);
    return;
  ...
  if (process.platform === "win32") {
    runtime.log(
      [
        "Windows detected — Nsemclaw runs great on WSL2!",
        "Native Windows might be trickier.",
        "Quick setup: wsl --install (one command, one reboot)",
        "Guide: https://docs.nsemclaw.ai/windows",
      ].join("\n"),
    );
  }
