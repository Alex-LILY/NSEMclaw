...
export const resolveNsemclawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadNsemclawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/nsemclaw-root.js", () => ({
  resolveNsemclawPackageRoot,
}));
...
beforeEach(() => {
  ...
  resolveNsemclawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.NSEMCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.NSEMCLAW_UPDATE_IN_PROGRESS;
  process.env.NSEMCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nsemclaw-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.NSEMCLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.NSEMCLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.NSEMCLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
