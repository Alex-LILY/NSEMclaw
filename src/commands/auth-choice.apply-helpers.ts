...
async function resolveApiKeyRefForOnboarding(params: {
  ...
  while (true) {
    ...
    const source: SecretRefChoice = sourceRaw === "提供商" ? "提供商" : "env";
    sourceChoice = source;
    ...
    if (source === "env") {
      ...
      await params.prompter.note(
        `Validated environment variable ${envVar}. Nsemclaw will store a reference, not the key value.`,
        "Reference validated",
      );
    ...
    try {
      ...
      await params.prompter.note(
        `Validated ${providerEntry.source} reference ${selectedProvider}:${id}. Nsemclaw will store a reference, not the key value.`,
        "Reference validated",
      );
      return { ref, resolvedValue };
    } catch (error) {
    ...
    options: [
      {
        ...
        hint: "Stores the key directly in Nsemclaw config",
      },
      {
        value: "ref",
