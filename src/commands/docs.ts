...
const SEARCH_TOOL = "https://docs.nsemclaw.ai/mcp.SearchNsemclaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.nsemclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('nsemclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.nsemclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('nsemclaw docs "your query"')}`);
    }
    return;
  }
