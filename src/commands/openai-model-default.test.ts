...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: NsemclawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: NsemclawConfig) => { changed: boolean; next: NsemclawConfig };
  defaultModel: string;
  overrideConfig: NsemclawConfig;
  alreadyDefaultConfig: NsemclawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: NsemclawConfig) => config,
      applyDefaultConfig: (config: NsemclawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: NsemclawConfig = { agents: { defaults: {} } };
