...
function resolveAgentName(cfg: NsemclawConfig, agentId: string) {
...
function resolveAgentModel(cfg: NsemclawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: NsemclawConfig): AgentSummary[] {
