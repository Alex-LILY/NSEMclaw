...
import type { NsemclawConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: NsemclawConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("nsemclaw security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.nsemclaw/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("nsemclaw approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("nsemclaw configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("nsemclaw config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("nsemclaw doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("nsemclaw config set gateway.bind loopback")}`,
  ...
  const warnDmPolicy = async (params: {
    ...
    if (dmPolicy === "已禁用") {
      warnings.push(`- ${params.label} DMs: disabled (${policyPath}="已禁用").`);
      return;
    }
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('nsemclaw config set session.dmScope "per-channel-peer"') +
