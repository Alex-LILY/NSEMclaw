...
export async function statusCommand(
  ...
  const defaultCtx = defaults.contextTokens
    ...
    : "";
  const eventsValue =
    summary.queuedSystemEvents.length > 0 ? `${summary.queuedSystemEvents.length} queued` : "无";

  const probesValue = health ? ok("已启用") : muted("skipped (use --deep)");
  ...
  const memoryValue = (() => {
    ...
    if (!memory) {
      const slot = memoryPlugin.slot ? `plugin ${memoryPlugin.slot}` : "插件";
  ...
  const overviewRows = [
    ...
    { Item: "节点服务", Value: nodeDaemonValue },
    { Item: "智能体", Value: agentsValue },
  ...
  runtime.log(theme.heading("瑶姬状态"));
  runtime.log("");
  ...
  runtime.log(
    renderTable({
      width: tableWidth,
      columns: [
        { key: "项目", header: "项目", minWidth: 12 },
        { key: "值", header: "值", flex: true, minWidth: 32 },
      ],
      rows: overviewRows,
    }).trimEnd(),
  );
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`nsemclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("nsemclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("nsemclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("nsemclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("nsemclaw security audit --deep")}`));
  ...
  if (health) {
    ...
    for (const line of formatHealthChannelLines(health, { accountMode: "all" })) {
      ...
      const status = (() => {
        ...
        if (normalized.startsWith("未配置")) {
          return muted("OFF");
        }
        if (normalized.startsWith("已配置")) {
          return ok("正常");
        }
        if (normalized.startsWith("已链接")) {
          return ok("LINKED");
        }
        if (normalized.startsWith("未链接")) {
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.nsemclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.nsemclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("nsemclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("nsemclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("nsemclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("nsemclaw gateway probe")}`);
  }
}
