...
describe("shared/frontmatter", () => {
  ...
  test("resolveNsemclawManifestBlock parses JSON5 metadata and picks openclaw block", () => {
    const frontmatter = {
      metadata: "{ openclaw: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveNsemclawManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveNsemclawManifestBlock returns undefined for invalid input", () => {
    expect(resolveNsemclawManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveNsemclawManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveNsemclawManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
