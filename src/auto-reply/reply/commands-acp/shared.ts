...
export function resolveConfiguredAcpBackendId(cfg: NsemclawConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: NsemclawConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(localPath)) {
      return `nsemclaw plugins install ${localPath}`;
    }
    return "nsemclaw plugins install @openclaw/acpx";
  }
