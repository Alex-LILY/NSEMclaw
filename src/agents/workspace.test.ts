...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses NSEMCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      NSEMCLAW_HOME: "/srv/nsemclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/nsemclaw-home"), ".nsemclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".nsemclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("nsemclaw-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "nsemclaw-workspace-hardlink-"));
    try {
