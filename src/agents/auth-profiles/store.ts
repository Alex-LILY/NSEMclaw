 ...
 * A common mistake — caused by the similarity with the `nsemclaw.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (nsemclaw.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.NSEMCLAW_AUTH_STORE_READONLY === "1";
