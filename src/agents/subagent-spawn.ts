...
const log = createSubsystemLogger("subagent-spawn");
...
export async function spawnSubagentDirect(
  ...
  // 记忆继承初始化
  // ============================================================================
  const parentAgentId = params.parentAgentId ?? requesterAgentId;
  if (parentAgentId && parentAgentId !== targetAgentId) {
    try {
      const { createPersistentSelectiveMemoryInheritance } =
        await import("../cognitive-core/memory/PersistentSelectiveMemoryInheritance.js");
      const { getInheritanceSystem } = await import("./tools/cognitive-core-tool.js");

      // 获取父 Agent 的记忆继承系统
      // 注意：如果父 Agent 是主 Agent，可能返回 null，这是正常的
      const parentInheritance = getInheritanceSystem(parentAgentId);

      // 创建子 Agent 的持久化记忆继承系统
      const childInheritance = createPersistentSelectiveMemoryInheritance(targetAgentId, {
        strategy: params.memoryInheritanceStrategy ?? "filtered",
      ...
      // 从父 Agent 获取可继承的共享记忆
      // 如果父 Agent 是主 Agent（没有继承系统），则跳过记忆继承
      const sharedMemories = parentInheritance
        ? (await (
            parentInheritance as import("../cognitive-core/memory/PersistentSelectiveMemoryInheritance.js").PersistentSelectiveMemoryInheritance
          ).getAllShared?.({
            minImportance: params.minInheritedImportance ?? 0.5,
            tags: ["inheritable"],
          })) ?? []
        : [];
      ...
      if (sharedMemories.length > 0) {
        // 将共享记忆转换为 MemAtom 格式
        const memoriesToInherit = sharedMemories.map((m) => ({
          id: m.id,
          contentHash: "",
          content: m.content,
          contentType: (m.type || "fact") as import("../cognitive-core/types/index.js").ContentType,
          embedding: [] as number[],
          temporal: {
            created: m.createdAt,
        ...
        await childInheritance.inheritFromParent(parentAgentId, memoriesToInherit, {
          strategy: params.memoryInheritanceStrategy ?? "filtered",
          maxMemories: 1000,
          decayFactor: 0.9,
        });
        ...
        log.info(
          `Memory inheritance initialized for ${targetAgentId} from ${parentAgentId}: ${memoriesToInherit.length} memories`,
        );
      } else {
        log.info(`No inheritable memories found for ${targetAgentId} from ${parentAgentId}`);
      }
    } catch (error) {
      // 记忆继承失败不应阻止 Agent 创建
      log.warn(`Memory inheritance failed for ${targetAgentId}: ${error}`);
    }
  }
