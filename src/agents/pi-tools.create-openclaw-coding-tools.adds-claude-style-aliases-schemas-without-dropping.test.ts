...
import { createNsemclawTools } from "./nsemclaw-tools.js";
import { __testing, createNsemclawCodingTools } from "./pi-tools.js";
import { createNsemclawReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
...
describe("createNsemclawCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createNsemclawCodingTools({ senderIsOwner: true }).find(
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createNsemclawTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createNsemclawCodingTools({ messageProvider: "discord" });
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    ...
    const googleTools = createNsemclawCodingTools({
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createNsemclawReadTool(
      baseRead as unknown as Parameters<typeof createNsemclawReadTool>[0],
    );
