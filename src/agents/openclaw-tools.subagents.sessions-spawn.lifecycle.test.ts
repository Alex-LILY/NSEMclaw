...
} from "./nsemclaw-tools.subagents.sessions-spawn.test-harness.js";
import { resetSubagentRegistryForTests } from "./subagent-registry.js";

const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.NSEMCLAW_TEST_FAST;
...
describe("openclaw-tools: subagents (sessions_spawn lifecycle)", () => {
  ...
  afterAll(() => {
    if (fastModeEnv.previous === undefined) {
      delete process.env.NSEMCLAW_TEST_FAST;
      return;
    }
    process.env.NSEMCLAW_TEST_FAST = fastModeEnv.previous;
  });
