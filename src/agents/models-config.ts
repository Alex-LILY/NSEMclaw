...
type ModelsConfig = NonNullable<NsemclawConfig["models"]>;
...
export async function ensureNsemclawModelsJson(
  config?: NsemclawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveNsemclawAgentDir();
