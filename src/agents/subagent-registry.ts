...
/**
 * 清理指定 runId 的所有 timers
 */
function clearRunTimers(runId: string): void {
 ...
 * 设置 timer 并追踪
 */
function setRunTimer(runId: string, callback: () => void, delayMs: number): void {
  clearRunTimers(runId); // 先清理已有的
 ...
 * 清理所有 timers（用于测试或关闭）
 */
function clearAllRunTimers(): void {
  for (const [runId, timer] of activeTimers) {
    clearTimeout(timer);
    activeTimers.delete(runId);
  }
}
