...
import { resolveNsemclawAgentDir } from "./agent-paths.js";
import {
...
import { ensureNsemclawModelsJson } from "./models-config.js";
import { isRateLimitErrorMessage } from "./pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "./pi-model-discovery.js";

const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.NSEMCLAW_LIVE_TEST);
const DIRECT_ENABLED = Boolean(process.env.NSEMCLAW_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isTruthyEnvValue(process.env.NSEMCLAW_LIVE_REQUIRE_PROFILE_KEYS);
...
describeLive("live models (profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.NSEMCLAW_LIVE_MODELS?.trim();
      const useModern = rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseModelFilter(rawModels) : null;
      const allowNotFoundSkip = useModern;
      const providers = parseProviderFilter(process.env.NSEMCLAW_LIVE_PROVIDERS);
      const perModelTimeoutMs = toInt(process.env.NSEMCLAW_LIVE_MODEL_TIMEOUT_MS, 30_000);
      const maxModels = toInt(process.env.NSEMCLAW_LIVE_MAX_MODELS, 0);
