...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): NsemclawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as NsemclawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): NsemclawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as NsemclawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: NsemclawConfig, accountId = "media") {
    await handleTelegramAction(
