...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    openclaw: process.env.NSEMCLAW_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.openclaw === undefined) {
      ...
      process.env.NSEMCLAW_GATEWAY_TOKEN = envSnapshot.openclaw;
    }
  ...
  it("uses OPENCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
    process.env.NSEMCLAW_GATEWAY_TOKEN = "env-token";
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.NSEMCLAW_GATEWAY_TOKEN = "local-env-token";
