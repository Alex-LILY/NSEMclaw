...
function resolveDefaultModelRef(cfg?: NsemclawConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
function pickMaxBytes(cfg?: NsemclawConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: NsemclawConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureNsemclawModelsJson(effectiveCfg, params.agentDir);
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      for (const imageRawInput of imageInputs) {
        ...
        if (!isDataUrl) {
          throw new Error("Image file loading is not supported in this build");
        }
        const media: { buffer: Buffer; mimeType: string; kind: string } =
          decodeDataUrl(resolvedImage);
        if (media.kind !== "image") {
          throw new Error(`Unsupported media type: ${media.kind}`);
        }

        const mimeType: string = media.mimeType || "image/png";
