...
const defaultTools = createNsemclawCodingTools({ senderIsOwner: true });
...
describe("createNsemclawCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: NsemclawConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createNsemclawCodingTools({
    ...
    const anthropicTools = createNsemclawCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: NsemclawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createNsemclawCodingTools({
    ...
    const denied = createNsemclawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createNsemclawCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
