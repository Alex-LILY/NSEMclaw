...
import {
  ...
  normalizeStringList,
  parseNsemclawManifestInstallBase,
  parseFrontmatterBool,
  resolveNsemclawManifestBlock,
  resolveNsemclawManifestInstall,
  resolveNsemclawManifestOs,
  resolveNsemclawManifestRequires,
} from "../../shared/frontmatter.js";
import type {
  NsemclawSkillMetadata,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseNsemclawManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveNsemclawMetadata(
  frontmatter: ParsedSkillFrontmatter,
): NsemclawSkillMetadata | undefined {
  const metadataObj = resolveNsemclawManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveNsemclawManifestRequires(metadataObj);
  const install = resolveNsemclawManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveNsemclawManifestOs(metadataObj);
  return {
