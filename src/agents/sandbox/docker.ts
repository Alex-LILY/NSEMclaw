...
export async function execDockerRaw(
  ...
  if (!available) {
    const err = new Error("Docker not found in PATH. Please install Docker or check your installation.") as ExecDockerRawError;
    err.code = 127;
    err.stdout = Buffer.alloc(0);
    err.stderr = Buffer.from("Docker not found in PATH");
    throw err;
  }
  ...
  return new Promise<ExecDockerRawResult>((resolve, reject) => {
    ...
    child.on("error", (error) => {
      if ((error as NodeJS.ErrnoException).code === "ENOENT") {
        const err = new Error("Docker not found in PATH") as ExecDockerRawError;
        err.code = 127;
        err.stdout = Buffer.alloc(0);
        err.stderr = Buffer.from("Docker not found in PATH");
        reject(err);
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "nsemclaw.sandbox=1");
  args.push("--label", `nsemclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `nsemclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `nsemclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "nsemclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`nsemclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`nsemclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("nsemclaw sandbox recreate --all");
}
