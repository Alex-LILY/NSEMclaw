...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.NSEMCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".nsemclaw", `workspace-${profile}`);
  }
  return path.join(home, ".nsemclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".nsemclaw";
