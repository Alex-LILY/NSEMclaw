...
const GOOGLE_LIVE_SINGLE_KEY = "NSEMCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "NSEMCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "NSEMCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "NSEMCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `NSEMCLAW_LIVE_${base}_KEY`;
