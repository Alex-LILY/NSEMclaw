...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: NsemclawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: NsemclawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: NsemclawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: NsemclawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: NsemclawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: NsemclawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: NsemclawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("uses NSEMCLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "nsemclaw-home");
    vi.stubEnv("NSEMCLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as NsemclawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".nsemclaw", "workspace"));
  });

  it("uses NSEMCLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "nsemclaw-home");
    vi.stubEnv("NSEMCLAW_HOME", home);
    // Clear state dir so it falls back to NSEMCLAW_HOME
    vi.stubEnv("NSEMCLAW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as NsemclawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".nsemclaw", "agents", "main", "agent"));
  });
});
