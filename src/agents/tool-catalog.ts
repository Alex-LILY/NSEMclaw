...
type CoreToolDefinition = {
  ...
  includeInNsemclawGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const openclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInNsemclawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:nsemclaw": openclawTools,
