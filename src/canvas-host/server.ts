...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.nsemclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.nsemclawCanvasA2UIAction &&
        typeof window.nsemclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.nsemclawSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("nsemclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Nsemclaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.nsemclawSendUserAction === "function"
        ? window.nsemclawSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.NSEMCLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.NSEMCLAW_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Nsemclaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
