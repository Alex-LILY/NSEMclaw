...
describe("OPENCLAW_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    originalEnv = process.env.NSEMCLAW_LOG_LEVEL;
    delete process.env.NSEMCLAW_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.NSEMCLAW_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.NSEMCLAW_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.NSEMCLAW_LOG_LEVEL = "nope";
