...
import { resolvePreferredNsemclawTmpDir } from "../infra/tmp-nsemclaw-dir.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredNsemclawTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "nsemclaw.log"); // legacy single-file path

const LOG_PREFIX = "nsemclaw";
...
function resolveSettings(): ResolvedSettings {
  let cfg: NsemclawConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => NsemclawConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.NSEMCLAW_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[nsemclaw] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
