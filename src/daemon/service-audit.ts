...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.NSEMCLAW_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service NSEMCLAW_GATEWAY_TOKEN does not match gateway.auth.token in nsemclaw.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `nsemclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
