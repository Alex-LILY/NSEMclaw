...
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "nsemclaw-root");
...
describe("resolveNsemclawPackageRoot", () => {
  let resolveNsemclawPackageRoot: typeof import("./nsemclaw-root.js").resolveNsemclawPackageRoot;
  let resolveNsemclawPackageRootSync: typeof import("./nsemclaw-root.js").resolveNsemclawPackageRootSync;

  beforeAll(async () => {
    ({ resolveNsemclawPackageRoot, resolveNsemclawPackageRootSync } =
      await import("./nsemclaw-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    expect(resolveNsemclawPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    expect(resolveNsemclawPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveNsemclawPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });
  ...
  it("returns null for non-openclaw package roots", async () => {
    ...
    expect(resolveNsemclawPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveNsemclawPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveNsemclawPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
