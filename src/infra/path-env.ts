...
function candidateBinDirs(opts: EnsureNsemclawPathOpts): { prepend: string[]; append: string[] } {
  ...
  try {
    ...
    const siblingCli = path.join(execDir, "nsemclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.NSEMCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "nsemclaw"))) {
 ...
 */
export function ensureNsemclawCliOnPath(opts: EnsureNsemclawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.NSEMCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.NSEMCLAW_PATH_BOOTSTRAPPED = "1";
