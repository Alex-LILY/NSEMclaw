...
import { POSIX_NSEMCLAW_TMP_DIR, resolvePreferredNsemclawTmpDir } from "./tmp-nsemclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredNsemclawTmpDir>[0]>;
...
function resolveWithMocks(params: {
  ...
  const wrappedLstatSync = vi.fn((target: string) => {
    if (target === POSIX_NSEMCLAW_TMP_DIR) {
  ...
  const resolved = resolvePreferredNsemclawTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredNsemclawTmpDir", () => {
  it("prefers /tmp/openclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_NSEMCLAW_TMP_DIR);
  ...
  it("prefers /tmp/openclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_NSEMCLAW_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });
