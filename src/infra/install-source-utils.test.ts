...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    const archivePath = path.join(cwd, "nsemclaw-plugin-1.2.3.tgz");
    await fs.writeFile(archivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "nsemclaw-plugin@1.2.3",
          name: "nsemclaw-plugin",
          version: "1.2.3",
          filename: "nsemclaw-plugin-1.2.3.tgz",
    ...
    const result = await runPack("nsemclaw-plugin@1.2.3", cwd);
    ...
    expect(result).toEqual({
      ...
      metadata: {
        name: "nsemclaw-plugin",
        version: "1.2.3",
        resolvedSpec: "nsemclaw-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "nsemclaw-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("falls back to parsing final stdout line when npm json output is unavailable", async () => {
    const cwd = await createFixtureDir();
    const expectedArchivePath = path.join(cwd, "nsemclaw-plugin-1.2.3.tgz");
    await fs.writeFile(expectedArchivePath, "", "utf-8");
    mockPackCommandResult({
      stdout: "npm notice created package\nnsemclaw-plugin-1.2.3.tgz\n",
    });
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("nsemclaw-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    const cwd = await createFixtureDir();
    await fs.writeFile(path.join(cwd, "nsemclaw-plugin-demo-2.0.0.tgz"), "", "utf-8");
    mockPackCommandResult({
      stdout:
        ...
        JSON.stringify([
          {
            ...
            filename: "nsemclaw-plugin-demo-2.0.0.tgz",
          },
        ]),
    });
    ...
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "nsemclaw-plugin-demo-2.0.0.tgz"),
      metadata: {
