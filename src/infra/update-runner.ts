...
const DEFAULT_PACKAGE_NAME = "nsemclaw";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nsemclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "nsemclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("nsemclaw doctor", doctorArgv, gitRoot, { NSEMCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
