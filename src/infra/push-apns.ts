...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.NSEMCLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.NSEMCLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set NSEMCLAW_APNS_TEAM_ID and NSEMCLAW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.NSEMCLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.NSEMCLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.NSEMCLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set NSEMCLAW_APNS_PRIVATE_KEY_P8 or NSEMCLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading NSEMCLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createNsemclawPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    nsemclaw: createNsemclawPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
