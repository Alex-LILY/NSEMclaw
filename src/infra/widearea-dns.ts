...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.NSEMCLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*nsemclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "nsemclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "nsemclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "nsemclaw-gw");
  ...
  records.push(`_nsemclaw-gw._tcp IN PTR ${instanceLabel}._nsemclaw-gw._tcp`);
  records.push(`${instanceLabel}._nsemclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._nsemclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; nsemclaw-content-hash: ${contentHash}\n${contentBody}`;
}
