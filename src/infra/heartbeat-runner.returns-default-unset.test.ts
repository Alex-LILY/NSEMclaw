...
beforeAll(async () => {
  ...
  whatsappPlugin.config = {
    ...whatsappPlugin.config,
    resolveAllowFrom: ({ cfg }: { cfg: NsemclawConfig }) =>
      cfg.channels?.whatsapp?.allowFrom?.map((entry: string | number) => String(entry)) ?? [],
  };
...
describe("resolveHeartbeatPrompt", () => {
  it("uses default or trimmed override prompts", () => {
    const cases = [
      { cfg: {} as NsemclawConfig, expected: HEARTBEAT_PROMPT },
      {
        cfg: {
  ...
  it("resolves configured and forced session key overrides", async () => {
    ...
    try {
      const cases = typedCases<{
        ...
        peerId: string;
        message: string;
        applyOverride: (params: { cfg: NsemclawConfig; sessionKey: string }) => void;
      ...
      }>([
        {
          ...
          message: "Group alert",
          applyOverride: ({ cfg, sessionKey }: { cfg: NsemclawConfig; sessionKey: string }) => {
      ...
      for (const testCase of cases) {
        ...
        const sendWhatsApp = vi
          ...
          .mockResolvedValue({ messageId: "m1", roomId: "room1", toJid: "jid" });
