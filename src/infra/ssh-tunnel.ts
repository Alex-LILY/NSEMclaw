...
// SSH 可用性缓存
let sshAvailable: boolean | null = null;
let sshCheckPromise: Promise<boolean> | null = null;
...
async function isSshAvailable(): Promise<boolean> {
  ...
  if (sshCheckPromise) {
    return sshCheckPromise;
  }
  
  sshCheckPromise = new Promise((resolve) => {
    const child = spawn(SSH_PATH, ["-V"], { stdio: "ignore" });
    child.on("error", () => {
      sshAvailable = false;
      resolve(false);
    });
    child.on("close", (code) => {
      // ssh -V 返回 0 或 255 都是正常的（取决于 OpenSSH 版本）
      sshAvailable = code === 0 || code === 255;
      resolve(sshAvailable);
    });
    // 3秒超时
    setTimeout(() => {
      child.kill();
...
export async function startSshPortForward(opts: {
  ...
  // 检查 SSH 是否可用
  const sshAvailable = await isSshAvailable();
  if (!sshAvailable) {
    throw new Error(`SSH not found at ${SSH_PATH}. Please install OpenSSH client.`);
  }
  ...
  const stderr: string[] = [];
  const child = spawn(SSH_PATH, args, {
