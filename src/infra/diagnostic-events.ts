...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __nsemclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__nsemclawDiagnosticEventsState) {
    globalStore.__nsemclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__nsemclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: NsemclawConfig): boolean {
