...
import { triggerNsemclawRestart } from "./restart.js";
 ...
 * - NSEMCLAW_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
 * - otherwise: spawn detached child with current argv/execArgv, then caller exits
 */
export function restartGatewayProcessWithFreshPid(): GatewayRespawnResult {
  if (isTruthy(process.env.NSEMCLAW_NO_RESPAWN)) {
  ...
  if (isLikelySupervisedProcess(process.env)) {
    ...
    if (process.platform === "darwin" && process.env.NSEMCLAW_LAUNCHD_LABEL?.trim()) {
      const restart = triggerNsemclawRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
