...
import {
  ...
  // @ts-expect-error - Type may not be exported from channel-adapters
...
export async function buildCrossContextDecoration(params: {
  ...
  const componentsBuilder = (adapter as any).supportsComponentsV2
    ? (adapter as any).buildCrossContextComponents
      ? (message: string) =>
          (adapter as any).buildCrossContextComponents!({
            originLabel,
            message,
