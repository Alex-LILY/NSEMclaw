...
/**
 * Auto-inject Slack thread ID when the message tool targets
 * the same channel the session originated from.
 */
export function resolveSlackAutoThreadId(params: {
  ...
  // Simple comparison for Slack targets (e.g., "user:U123" or "C12345")
  const normalizeSlackId = (id: string): string => {
    const parts = id.split(":");
    return parts[parts.length - 1]?.toLowerCase() ?? id.toLowerCase();
  };
  if (normalizeSlackId(params.to) !== normalizeSlackId(context.currentChannelId)) {
    return undefined;
  }
...
async function hydrateAttachmentPayload(params: {
  ...
  if (!params.dryRun && !readStringParam(params.args, "buffer", { trim: false }) && mediaSource) {
    ...
    const loadOptions = buildAttachmentMediaLoadOptions({ policy: params.mediaPolicy, maxBytes });
    let buffer: Buffer;
    if ("sandboxValidated" in loadOptions && loadOptions.sandboxValidated) {
      buffer = await loadOptions.readFile(mediaSource);
    } else {
      buffer = await fs.readFile(mediaSource);
    }
    params.args.buffer = buffer.toString("base64");
    if (!filename) {
