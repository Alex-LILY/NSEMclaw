...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: NsemclawConfig | null = null;
  ...
  get(key: string, cfg: NsemclawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: NsemclawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: NsemclawConfig): void {
