...
export function triggerNsemclawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.NSEMCLAW_SYSTEMD_UNIT,
        process.env.NSEMCLAW_PROFILE,
      );
  ...
  const label =
    process.env.NSEMCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.NSEMCLAW_PROFILE);
