...
describe("restartGatewayProcessWithFreshPid", () => {
  ...
  it("runs launchd kickstart helper on macOS when launchd label is set", () => {
    setPlatform("darwin");
    process.env.LAUNCH_JOB_LABEL = "ai.openclaw.gateway";
    process.env.NSEMCLAW_LAUNCHD_LABEL = "ai.openclaw.gateway";
  ...
  it("spawns detached child with current exec argv", () => {
    delete process.env.NSEMCLAW_NO_RESPAWN;
  ...
  it("returns supervised when OPENCLAW_SYSTEMD_UNIT is set", () => {
    clearSupervisorHints();
    process.env.NSEMCLAW_SYSTEMD_UNIT = "openclaw-gateway.service";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(spawnMock).not.toHaveBeenCalled();
  });
  ...
  it("returns supervised when OPENCLAW_SERVICE_MARKER is set", () => {
    clearSupervisorHints();
    process.env.NSEMCLAW_SERVICE_MARKER = "gateway";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(spawnMock).not.toHaveBeenCalled();
  });
