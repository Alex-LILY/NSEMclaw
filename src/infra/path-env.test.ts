...
let ensureNsemclawCliOnPath: typeof import("./path-env.js").ensureNsemclawCliOnPath;

describe("ensureNsemclawCliOnPath", () => {
  const envKeys = [
    "PATH",
  ...
  beforeAll(async () => {
    ({ ensureNsemclawCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling openclaw exists", () => {
    ...
    delete process.env.NSEMCLAW_PATH_BOOTSTRAPPED;
