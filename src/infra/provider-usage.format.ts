...
function formatWindowShort(window: UsageWindow, now?: number): string {
  ...
  return `剩余 ${remaining.toFixed(0)}% (${window.label}${resetSuffix})`;
}
...
export function formatUsageWindowSummary(
  ...
  const parts = windows.map((window) => {
    ...
    const resetSuffix = reset ? ` ⏱${reset}` : "";
    return `${window.label} 剩余 ${remaining.toFixed(0)}%${resetSuffix}`;
  });
  return parts.join(" · ");
}
...
export function formatUsageReportLines(summary: UsageSummary, opts?: { now?: number }): string[] {
  if (summary.providers.length === 0) {
    return ["使用: 无提供商使用数据可用。"];
  }

  const lines: string[] = ["使用:"];
  for (const entry of summary.providers) {
    ...
    if (entry.windows.length === 0) {
      lines.push(`  ${entry.displayName}${planSuffix}: 无数据`);
      continue;
    }
    ...
    for (const window of entry.windows) {
      ...
      lines.push(`    ${window.label}: 剩余 ${remaining.toFixed(0)}%${resetSuffix}`);
    }
  }
  return lines;
}
