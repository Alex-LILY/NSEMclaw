...
export function resolveTtsConfig(cfg: NsemclawConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.NSEMCLAW_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: NsemclawConfig): string | undefined {
  ...
  for (const provider of providers) {
    ...
    try {
      if (provider === "edge") {
        ...
        const tempRoot = resolvePreferredNsemclawTmpDir();
...
export async function maybeApplyTtsToPayload(params: {
  ...
  // Note: markdown stripping for TTS removed with LINE module
