...
declare const __NSEMCLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "nsemclaw";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["NSEMCLAW_VERSION"],
      env["NSEMCLAW_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Nsemclaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __NSEMCLAW_VERSION__ === "string" && __NSEMCLAW_VERSION__) ||
  process.env.NSEMCLAW_BUNDLED_VERSION ||
