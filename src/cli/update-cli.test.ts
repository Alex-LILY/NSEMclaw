...
import type { NsemclawConfig, ConfigFileSnapshot } from "../config/types.nsemclaw.js";
...
vi.mock("../infra/nsemclaw-root.js", () => ({
  resolveNsemclawPackageRoot: vi.fn(),
}));
...
const { resolveNsemclawPackageRoot } = await import("../infra/nsemclaw-root.js");
...
describe("update-cli", () => {
  ...
  const baseConfig = {} as NsemclawConfig;
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveNsemclawPackageRoot).mockResolvedValue(root);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveNsemclawPackageRoot).mockClear();
    ...
    vi.mocked(resolveNsemclawPackageRoot).mockResolvedValue(process.cwd());
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as NsemclawConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveNsemclawPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ OPENCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        ...
        expect(process.env.NSEMCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
