...
function normalizeEnvironment(value: unknown): "沙盒" | "production" | null {
  if (typeof value !== "string") {
    return null;
  }
  const normalized = value.trim().toLowerCase();
  if (normalized === "沙盒" || normalized === "production") {
    return normalized;
  }
  return null;
}
...
export function registerNodesPushCommand(nodes: Command) {
  nodesCallOpts(
    nodes
      ...
      .option("--title <text>", "Push title", "Nsemclaw")
      ...
      .action(async (opts: NodesPushOpts) => {
        await runNodesCommand("push", async () => {
          ...
          const title = String(opts.title ?? "").trim() || "Nsemclaw";
