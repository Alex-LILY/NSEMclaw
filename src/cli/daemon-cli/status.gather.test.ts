...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    NSEMCLAW_STATE_DIR: "/tmp/openclaw-daemon",
    NSEMCLAW_CONFIG_PATH: "/tmp/openclaw-daemon/nsemclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.NSEMCLAW_STATE_DIR ?? "/tmp/openclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.NSEMCLAW_CONFIG_PATH ?? `${stateDir}/nsemclaw.json`;
});
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.NSEMCLAW_STATE_DIR = "/tmp/openclaw-cli";
    process.env.NSEMCLAW_CONFIG_PATH = "/tmp/openclaw-cli/nsemclaw.json";
