...
export function registerChannelsCli(program: Command) {
  ...
  const channels = program
    .command("channels")
    .description("管理已连接的聊天频道和账户")
    .addHelpText(
      "after",
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nsemclaw channels list", "列出已配置的频道和认证档案。"],
          ["nsemclaw channels status --probe", "运行频道状态检查和探测。"],
          [
            "nsemclaw channels add --channel telegram --token <token>",
            "非交互式添加或更新频道账户。",
          ],
  ...
  channels
    .command("login")
    .description("连接频道账户 (如果支持)")
    .option("--channel <channel>", "频道别名 (仅配置一个时自动选择)")
    .option("--account <id>", "Account id (accountId)")
