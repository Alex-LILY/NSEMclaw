...
export function registerCronAddCommand(cron: Command) {
  addGatewayClientOptions(
    cron
      ...
      .option("--wake <mode>", "Wake mode (now|next-heartbeat)", "现在")
      ...
      .action(async (opts: GatewayRpcOpts & Record<string, unknown>, cmd?: Command) => {
        try {
          ...
          const wakeModeRaw = typeof opts.wake === "string" ? opts.wake : "现在";
          const wakeMode = wakeModeRaw.trim() || "现在";
          if (wakeMode !== "现在" && wakeMode !== "next-heartbeat") {
          ...
          const optionSource =
            typeof cmd?.getOptionValueSource === "function"
              ...
              : () => undefined;
          const sessionSource = optionSource("会话");
