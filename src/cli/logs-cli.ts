...
async function fetchLogs(
  ...
  if (!payload || typeof payload !== "object") {
    throw new Error("意外的 logs.tail 响应");
  }
...
function createLogWriters() {
  const writer = createSafeStreamWriter({
    ...
    onBrokenPipe: (err, stream) => {
      ...
      const message = `nsemclaw logs: 输出 ${target} 已关闭 (${code})。停止跟踪。`;
      try {
...
function emitGatewayError(
  ...
  const message = "网关不可达。它是否正在运行且可访问？";
  const hint = `提示: 运行 \`${formatCliCommand("nsemclaw 诊断")}\`。`;
...
export function registerLogsCli(program: Command) {
  const logs = program
    ...
    .option("--follow", "跟踪日志输出", false)
    .option("--interval <ms>", "轮询间隔 (毫秒)", "1000")
    .option("--json", "输出 JSON 日志行", false)
    .option("--plain", "纯文本输出 (无 ANSI 样式)", false)
    .option("--no-color", "禁用 ANSI 颜色")
    .option("--local-time", "在本地时区显示时间戳", false)
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/logs", "docs.nsemclaw.ai/cli/logs")}\n`,
    );
  ...
  logs.action(async (opts: LogsCliOptions) => {
    ...
    while (true) {
      ...
      if (jsonMode) {
        ...
        if (payload.truncated) {
          if (!errorLine("日志跟踪已截断 (增加 --max-bytes)。")) {
            return;
          }
        }
        if (payload.reset) {
