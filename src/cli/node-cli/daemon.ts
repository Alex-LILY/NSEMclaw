...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("nsemclaw node install"), formatCliCommand("nsemclaw node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("nsemclaw node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    NSEMCLAW_LOG_PREFIX: baseEnv.NSEMCLAW_LOG_PREFIX ?? "node",
