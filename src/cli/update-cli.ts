...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Nsemclaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["nsemclaw update", "Update a source checkout (git)"],
        ["nsemclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["nsemclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["nsemclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["nsemclaw update --dry-run", "Preview actions without changing anything"],
        ["nsemclaw update --no-restart", "Update without restarting the service"],
        ["nsemclaw update --json", "Output result as JSON"],
        ["nsemclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["nsemclaw update wizard", "Interactive update wizard"],
        ["nsemclaw --update", "Shorthand for nsemclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run nsemclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.nsemclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nsemclaw update status", "Show channel + version status."],
          ["nsemclaw update status --json", "JSON output."],
          ["nsemclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.nsemclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
