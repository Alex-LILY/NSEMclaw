...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.nsemclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Nsemclaw hooks")
    ...
    .option("--hook-url <url>", "Nsemclaw hook URL")
    .option("--hook-token <token>", "Nsemclaw hook token")
