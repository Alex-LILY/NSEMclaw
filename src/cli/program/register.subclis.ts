...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.NSEMCLAW_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.NSEMCLAW_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<NsemclawConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "channels",
    description: "管理连接的聊天频道（Telegram、Discord 等）",
    hasSubcommands: true,
    register: async (program) => {
  ...
  {
    name: "directory",
    description: "查找支持的聊天频道的联系人和群组 ID（自己、同伴、群组）",
    hasSubcommands: true,
    register: async (program) => {
