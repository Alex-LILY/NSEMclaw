...
import type { ConfigFileSnapshot, NsemclawConfig } from "../config/types.js";
...
const mockWriteConfigFile = vi.fn<
  (cfg: NsemclawConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: NsemclawConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: NsemclawConfig, config: NsemclawConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: NsemclawConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
  ...
  describe("config set parsing flags", () => {
    ...
    it("shows --strict-json and keeps --json as a legacy alias in help", async () => {
      ...
      expect(helpText).toContain("--strict-json 的旧别名");
    });
  });
