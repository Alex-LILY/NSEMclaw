...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const previousStateDir = process.env.NSEMCLAW_STATE_DIR;

    const tempHome = await createTempHomeEnv("nsemclaw-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.NSEMCLAW_STATE_DIR).toBe(path.join(tempHome.home, ".nsemclaw"));
    ...
    expect(process.env.NSEMCLAW_STATE_DIR).toBe(previousStateDir);
