...
function listAgents(cfg: NsemclawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: NsemclawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: NsemclawConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<NsemclawConfig, EvaluatedBindingsCache>();
