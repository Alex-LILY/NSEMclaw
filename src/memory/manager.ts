...
export class MemoryIndexManager extends MemoryManagerEmbeddingOps implements MemorySearchManager {
  ...
  protected readonly cfg: NsemclawConfig;
  ...
  static async get(params: {
    ...
    const createPromise = (async () => {
      ...
      // 如果数据库为空，同步触发初始索引
      if (!params.purpose || params.purpose === "default") {
        const fileCount = manager.getFileCount();
        const hasMemorySource = settings.sources.includes("memory");
        log.info(`Memory manager created for ${agentId}: ${fileCount} files, sources=${settings.sources.join(",")}`);
        
        if (fileCount === 0 && hasMemorySource) {
          log.info(`Database empty, triggering initial sync with force=true...`);
          try {
            await manager.sync({ reason: "startup", force: true });
            const afterCount = manager.getFileCount();
            log.info(`Initial sync completed: ${afterCount} files indexed`);
          } catch (err) {
            log.error(`Initial sync failed: ${String(err)}`);
          }
        }
      }
  ...
  status(): MemoryProviderStatus {
    ...
    // 检测存储格式：优先查询 NSEM vectors 表，回退到旧 chunks 表
    let vectorCount = 0;
    ...
    try {
      ...
      if (vectorTableExists) {
        const vectors = this.db
          .prepare(`SELECT COUNT(*) as c FROM vectors WHERE 1=1${sourceFilter.sql}`)
    ...
    const sourceCounts = (() => {
      ...
      try {
        ...
        const tableName = vectorTableExists ? "vectors" : "chunks";
        const chunkRows = this.db
          .prepare(
            `SELECT source, COUNT(*) as c FROM ${tableName} WHERE 1=1${sourceFilter.sql} GROUP BY source`,
          )
          .all(...sourceFilter.params) as Array<{ source: MemorySource; c: number }>;
  ...
  async close(): Promise<void> {
    ...
    if (this.unifiedSessionIngestionUnregister) {
      this.unifiedSessionIngestionUnregister();
      this.unifiedSessionIngestionUnregister = null;
    }
    if (pendingSync) {
      try {
        await pendingSync;
