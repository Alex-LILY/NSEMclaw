...
export abstract class MemoryManagerEmbeddingOps extends MemoryManagerSyncOps {
  ...
  protected async indexFile(
    ...
    // Chunk the content using current settings
    let chunks = chunkMarkdown(content, this.settings.chunking).filter(
      (chunk) => chunk.text.trim().length > 0,
    );
    
    // Apply provider-specific token limits if provider is available
    if (this.provider) {
    ...
    const embeddings: number[][] = this.provider
      ? this.batch.enabled
    ...
    // Clean up old vector entries for this file
    if (vectorReady) {
      try {
        this.db
          .prepare(
    ...
    // Clean up old FTS entries for this file
    if (this.fts.enabled && this.fts.available) {
      try {
        this.db
          .prepare(`DELETE FROM ${FTS_TABLE} WHERE path = ? AND source = ? AND model = ?`)
          .run(entry.path, options.source, modelName);
      } catch {}
    }
