...
export type ResolvedQmdConfig = {
  ...
  chunking: ResolvedQmdChunkingConfig;
};

export type ResolvedQmdChunkingConfig = {
  tokens: number;
  overlap: number;
};
...
const DEFAULT_QMD_CHUNKING: ResolvedQmdChunkingConfig = {
  tokens: 1000,
  overlap: 100,
};
...
function resolveCustomPaths(
  ...
  rawPaths.forEach((entry, index) => {
    ...
    // 使用用户指定的名称，如果未指定则生成一个带 agentId 的唯一名称
    const userName = entry.name?.trim();
    const baseName = userName
      ? sanitizeName(userName)  // 直接使用用户指定的名称
      : scopeCollectionBase(`custom-${index + 1}`, agentId);  // 未指定时才加 agentId 后缀
...
function resolveChunking(raw?: { tokens?: number; overlap?: number }): ResolvedQmdChunkingConfig {
  const parsed: ResolvedQmdChunkingConfig = { ...DEFAULT_QMD_CHUNKING };
  if (!raw) {
    return parsed;
  }
  if (typeof raw.tokens === "number" && raw.tokens > 0) {
    parsed.tokens = Math.floor(raw.tokens);
  }
  if (typeof raw.overlap === "number" && raw.overlap >= 0) {
    parsed.overlap = Math.floor(raw.overlap);
  }
  return parsed;
}
...
export function resolveMemoryBackendConfig(params: {
  ...
  const resolved: ResolvedQmdConfig = {
    ...
    chunking: resolveChunking(qmdCfg?.chunking),
  };
