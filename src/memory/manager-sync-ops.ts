...
export abstract class MemoryManagerSyncOps {
  protected abstract readonly cfg: NsemclawConfig;
  ...
  protected unifiedSessionIngestionUnregister: (() => void) | null = null;
  ...
  protected ensureSessionListener() {
    ...
    // 如果启用了统一会话摄入，使用 UnifiedSessionIngestionManager
    if (this.shouldUseUnifiedSessionIngestion()) {
      this.ensureUnifiedSessionListener();
    } else {
      // 使用传统的事件监听
      this.ensureLegacySessionListener();
    }
  }

  /**
   * 判断是否使用统一会话摄入系统
   * 当 NSEM 和 Builtin Memory 都启用时，使用统一系统避免重复监听
   */
  private shouldUseUnifiedSessionIngestion(): boolean {
    // 检查是否启用了 NSEM（通过配置检测）
    const nsemEnabled = this.isNSEMEnabledForAgent();
    // 当 NSEM 启用时，使用统一系统
    return nsemEnabled;
  }

  /**
   * 检查是否为当前 agent 启用了 NSEM
   */
  private isNSEMEnabledForAgent(): boolean {
    // 通过配置检查 NSEM 是否启用
    const agentList = this.cfg.agents?.list as Array<{ id: string; nsem?: { enabled?: boolean } }> | undefined;
    const agentConfig = agentList?.find((a) => a.id === this.agentId);
    const defaultConfig = (this.cfg.agents?.defaults as any)?.nsem;

    // 检查 agent 配置或默认配置
    const nsemConfig = agentConfig?.nsem ?? defaultConfig;
    return nsemConfig?.enabled !== false;
  }

  /**
   * 使用传统的独立事件监听（与 NSEM 分开）
   */
  private ensureLegacySessionListener() {
  ...
  /**
   * 使用 UnifiedSessionIngestionManager（与 NSEM 共享）
   */
  private ensureUnifiedSessionListener() {
    if (this.unifiedSessionIngestionUnregister) {
      return;
    }

    const manager = getUnifiedSessionIngestionManager(this.agentId, {
      debounceMs: this.settings.sync?.watchDebounceMs ?? 5000,
      deltaBytes: this.settings.sync?.sessions?.deltaBytes ?? 4096,
      deltaMessages: this.settings.sync?.sessions?.deltaMessages ?? 3,
    });

    // 创建 Builtin Memory 消费者
    const consumer: SessionConsumer = {
      name: `builtin-memory-${this.agentId}`,

      consumeDelta: async (filePath, delta) => {
        if (delta.newMessages.length === 0) return;

        // 将增量添加到 dirty files
        this.sessionsDirtyFiles.add(filePath);
      ...
      syncFull: async (progress) => {
        // 触发全量同步
        await this.sync({ reason: "manual", force: true, progress });
      },
    };

    this.unifiedSessionIngestionUnregister = manager.registerConsumer(consumer);

    // 启动管理器（如果还没有启动）
    manager.start();

    log.debug(`Builtin Memory 已注册到统一会话摄入系统 (agent: ${this.agentId})`);
  }
  ...
  protected async runSync(params?: {
    ...
    try {
      if (needsFullReindex) {
        if (
          process.env.NSEMCLAW_TEST_FAST === "1" &&
          process.env.NSEMCLAW_TEST_MEMORY_UNSAFE_REINDEX === "1"
        ) {
