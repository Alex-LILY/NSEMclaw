...
describe("config io paths", () => {
  it("uses ~/.nsemclaw/nsemclaw.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".nsemclaw", 19001);
  ...
  it("defaults to ~/.nsemclaw/nsemclaw.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".nsemclaw", "nsemclaw.json"));
    });
  });
  ...
  it("uses OPENCLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      ...
      expect(io.configPath).toBe(path.join(home, "svc-home", ".nsemclaw", "nsemclaw.json"));
    });
  });

  it("honors explicit NSEMCLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".nsemclaw", 20002, "custom.json");
      const io = createIoForHome(home, { NSEMCLAW_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("honors legacy CLAWDBOT_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".nsemclaw", 20003, "legacy-custom.json");
  ...
  it("normalizes safe-bin config entries at config load time", async () => {
    await withTempHome(async (home) => {
      ...
      const configPath = path.join(configDir, "nsemclaw.json");
      await fs.writeFile(
        configPath,
        JSON.stringify(
          {
