...
import type { NsemclawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareNsemclawVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "NSEMCLAW_GATEWAY_TOKEN",
  "NSEMCLAW_GATEWAY_PASSWORD",
];
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  nsemclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  nsemclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: NsemclawConfig,
  pathSegments: string[],
): { changed: boolean; next: NsemclawConfig } {
...
function coerceConfig(value: unknown): NsemclawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as NsemclawConfig;
}
...
function stampConfigVersion(cfg: NsemclawConfig): NsemclawConfig {
...
function warnIfConfigFromFuture(cfg: NsemclawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareNsemclawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Nsemclaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as NsemclawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): NsemclawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as NsemclawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: NsemclawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as NsemclawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.NSEMCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.NSEMCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.NSEMCLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.NSEMCLAW_WATCH_SESSION === "string" &&
        deps.env.NSEMCLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.NSEMCLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.NSEMCLAW_WATCH_COMMAND === "string" &&
        deps.env.NSEMCLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.NSEMCLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `NSEMCLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: NsemclawConfig | null = null;
let runtimeConfigSourceSnapshot: NsemclawConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.NSEMCLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.NSEMCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): NsemclawConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): NsemclawConfig {
...
export async function writeConfigFile(
  ...
  // 排队机制：等待前一个写入完成
  const waitForPrev = configWriteQueue;
  
  // 创建新的写入 Promise
  const currentWrite = (async () => {
    await waitForPrev; // 等待队列中前一个写入
  ...
  // 更新队列
  configWriteQueue = currentWrite.catch(() => {
    // 忽略错误，让调用者处理
  });
