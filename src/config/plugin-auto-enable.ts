...
function isWhatsAppConfigured(cfg: NsemclawConfig): boolean {
...
function isGenericChannelConfigured(cfg: NsemclawConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: NsemclawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: NsemclawConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: NsemclawConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: NsemclawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: NsemclawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: NsemclawConfig, pluginId: string): NsemclawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
