...
export type ConfigSchema = ReturnType<typeof NsemclawSchema.toJSONSchema>;
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = NsemclawSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "NsemclawConfig";
  const hints = applyDerivedTags(mapSensitivePaths(NsemclawSchema, "", buildBaseHints()));
  const next = {
