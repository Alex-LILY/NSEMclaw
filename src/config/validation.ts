...
import type { NsemclawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: NsemclawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: NsemclawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = NsemclawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as NsemclawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as NsemclawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as NsemclawConfig,
  };
}
