...
describe("config io write", () => {
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".nsemclaw", "nsemclaw.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".nsemclaw", "logs", "config-audit.jsonl");
  ...
  it('shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withTempHome("openclaw-config-io-", async (home) => {
      ...
      const invalidConfig: NsemclawConfig = {
        channels: {
          telegram: {
            dmPolicy: "open",
            allowFrom: [],
  ...
  it("does not mutate caller config when unsetPaths is applied on first write", async () => {
    await withTempHome("openclaw-config-io-", async (home) => {
      const configPath = path.join(home, ".nsemclaw", "nsemclaw.json");
      const io = createConfigIO({
        env: {} as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
