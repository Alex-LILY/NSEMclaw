...
// Keep integration tests deterministic: never read a real nsemclaw.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.NSEMCLAW_SESSION_CACHE_TTL_MS;
    process.env.NSEMCLAW_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.NSEMCLAW_SESSION_CACHE_TTL_MS;
    } else {
      process.env.NSEMCLAW_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
