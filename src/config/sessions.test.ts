...
describe("sessions", () => {
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ NSEMCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("derives session transcripts dir from NSEMCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { NSEMCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.nsemclaw");
  ...
  it("resolves cross-agent paths when NSEMCLAW_STATE_DIR differs from stored paths", () => {
  ...
  it("falls back to derived transcript path when sessionFile is outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.nsemclaw"), () => {
      const sessionFile = resolveSessionFilePath(
        "sess-1",
      ...
      expect(sessionFile).toBe(
        path.join(
          path.resolve("/home/user/.nsemclaw"),
          "agents",
          "bot1",
          "sessions",
          "sess-1.jsonl",
