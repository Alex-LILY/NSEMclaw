...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.NSEMCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.NSEMCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.NSEMCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.NSEMCLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.NSEMCLAW_GATEWAY_TOKEN;
      process.env.NSEMCLAW_GATEWAY_TOKEN = "secret";
      port = await getFreePort();
      server = await startGatewayServer(port);
    });
