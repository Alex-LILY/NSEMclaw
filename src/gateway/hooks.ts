...
export function resolveHooksConfig(cfg: NsemclawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: NsemclawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-nsemclaw-token"] === "string"
      ? req.headers["x-nsemclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
