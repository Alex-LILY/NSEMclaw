...
const CLI_LIVE = isTruthyEnvValue(process.env.NSEMCLAW_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.NSEMCLAW_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.NSEMCLAW_LIVE_CLI_BACKEND_RESUME_PROBE);
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    ...
    const rawModel = process.env.NSEMCLAW_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    ...
    const cliCommand = process.env.NSEMCLAW_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(
    ...
    const baseCliArgs =
      parseJsonStringArray(
        ...
        process.env.NSEMCLAW_LIVE_CLI_BACKEND_ARGS,
    ...
    const cliClearEnv =
      parseJsonStringArray(
        ...
        process.env.NSEMCLAW_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.NSEMCLAW_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.NSEMCLAW_LIVE_CLI_BACKEND_IMAGE_MODE);
    ...
    const disableMcpConfig = process.env.NSEMCLAW_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
