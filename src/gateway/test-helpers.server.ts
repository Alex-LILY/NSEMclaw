...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "NSEMCLAW_SKIP_BROWSER_CONTROL_SERVER",
  "NSEMCLAW_SKIP_GMAIL_WATCHER",
  "NSEMCLAW_SKIP_CANVAS_HOST",
  "NSEMCLAW_BUNDLED_PLUGINS_DIR",
  "NSEMCLAW_SKIP_CHANNELS",
  "NSEMCLAW_SKIP_PROVIDERS",
  "NSEMCLAW_SKIP_CRON",
  "NSEMCLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nsemclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.NSEMCLAW_STATE_DIR = path.join(tempHome, ".nsemclaw");
  delete process.env.NSEMCLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.NSEMCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.NSEMCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.NSEMCLAW_SKIP_CANVAS_HOST = "1";
  process.env.NSEMCLAW_SKIP_CHANNELS = "1";
  process.env.NSEMCLAW_SKIP_PROVIDERS = "1";
  process.env.NSEMCLAW_SKIP_CRON = "1";
  process.env.NSEMCLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.NSEMCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "nsemclaw-test-no-bundled-extensions")
    : "nsemclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "nsemclaw-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".nsemclaw-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__nsemclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__nsemclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["NSEMCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.NSEMCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.NSEMCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.NSEMCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
