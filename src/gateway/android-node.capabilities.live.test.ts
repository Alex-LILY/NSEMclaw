...
const LIVE_ANDROID_NODE = isTruthyEnvValue(process.env.NSEMCLAW_LIVE_ANDROID_NODE);
...
function resolveGatewayConnection() {
  ...
  const urlOverride = readString(process.env.NSEMCLAW_ANDROID_GATEWAY_URL);
  const details = buildGatewayConnectionDetails({
    config: cfg,
    ...(urlOverride ? { url: urlOverride } : {}),
  });
  const tokenOverride = readString(process.env.NSEMCLAW_ANDROID_GATEWAY_TOKEN);
  const passwordOverride = readString(process.env.NSEMCLAW_ANDROID_GATEWAY_PASSWORD);
...
function selectTargetNode(nodes: NodeListNode[]): NodeListNode {
  const nodeIdOverride = readString(process.env.NSEMCLAW_ANDROID_NODE_ID);
  if (nodeIdOverride) {
  ...
  const nodeNameOverride = readString(process.env.NSEMCLAW_ANDROID_NODE_NAME)?.toLowerCase();
  if (nodeNameOverride) {
