...
import { resolveNsemclawAgentDir } from "../agents/agent-paths.js";
...
import { ensureNsemclawModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, NsemclawConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.NSEMCLAW_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.NSEMCLAW_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.NSEMCLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.NSEMCLAW_LIVE_GATEWAY_PROVIDERS);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.NSEMCLAW_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.NSEMCLAW_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<NsemclawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.NSEMCLAW_CONFIG_PATH,
    token: process.env.NSEMCLAW_GATEWAY_TOKEN,
    skipChannels: process.env.NSEMCLAW_SKIP_CHANNELS,
    skipGmail: process.env.NSEMCLAW_SKIP_GMAIL_WATCHER,
    skipCron: process.env.NSEMCLAW_SKIP_CRON,
    skipCanvas: process.env.NSEMCLAW_SKIP_CANVAS_HOST,
    agentDir: process.env.NSEMCLAW_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.NSEMCLAW_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.NSEMCLAW_SKIP_CHANNELS = "1";
  process.env.NSEMCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.NSEMCLAW_SKIP_CRON = "1";
  process.env.NSEMCLAW_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.NSEMCLAW_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveNsemclawAgentDir();
  ...
  process.env.NSEMCLAW_STATE_DIR = tempStateDir;
  ...
  process.env.NSEMCLAW_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.nsemclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveNsemclawAgentDir();
  const sanitizedCfg: NsemclawConfig = {
    ...params.cfg,
  ...
  const tempConfigPath = path.join(tempDir, "nsemclaw.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.NSEMCLAW_CONFIG_PATH = tempConfigPath;

  await ensureNsemclawModelsJson(nextCfg);
  ...
  try {
    ...
    process.env.NSEMCLAW_CONFIG_PATH = previous.configPath;
    process.env.NSEMCLAW_GATEWAY_TOKEN = previous.token;
    process.env.NSEMCLAW_SKIP_CHANNELS = previous.skipChannels;
    process.env.NSEMCLAW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.NSEMCLAW_SKIP_CRON = previous.skipCron;
    process.env.NSEMCLAW_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.NSEMCLAW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.NSEMCLAW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureNsemclawModelsJson(cfg);
      ...
      const rawModels = process.env.NSEMCLAW_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.nsemclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
