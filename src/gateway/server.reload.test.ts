...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.NSEMCLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.NSEMCLAW_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.NSEMCLAW_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.NSEMCLAW_SKIP_CHANNELS = "0";
    delete process.env.NSEMCLAW_SKIP_GMAIL_WATCHER;
    delete process.env.NSEMCLAW_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.NSEMCLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.NSEMCLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.NSEMCLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.NSEMCLAW_SKIP_PROVIDERS;
    } else {
      process.env.NSEMCLAW_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeEnvRefConfig() {
    const configPath = process.env.NSEMCLAW_CONFIG_PATH;
    if (!configPath) {
      throw new Error("NSEMCLAW_CONFIG_PATH is not set");
    }
    await fs.writeFile(
      configPath,
      `${JSON.stringify(
  ...
  async function writeAuthProfileEnvRefStore() {
    const stateDir = process.env.NSEMCLAW_STATE_DIR;
    if (!stateDir) {
      throw new Error("NSEMCLAW_STATE_DIR is not set");
    }
