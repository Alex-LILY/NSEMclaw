...
const { createNsemclawTools } = await import("../agents/nsemclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createNsemclawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createNsemclawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  ...
  process.env.NSEMCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.NSEMCLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("NSEMCLAW_CONFIG_PATH missing in gateway test environment");
      }
