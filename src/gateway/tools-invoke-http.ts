...
import { createNsemclawTools } from "../agents/nsemclaw-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-nsemclaw-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-nsemclaw-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-nsemclaw-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-nsemclaw-thread-id")?.trim() || undefined;
  ...
  const allTools = createNsemclawTools({
