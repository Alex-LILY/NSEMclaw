...
const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "nsemclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "nsemclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.NSEMCLAW_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.NSEMCLAW_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.NSEMCLAW_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[nsemclaw] Failed to respawn CLI:",
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[nsemclaw] ${parsed.error}`);
      process.exit(2);
    }
