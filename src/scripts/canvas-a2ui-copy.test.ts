...
describe("canvas a2ui copy", () => {
  ...
  it("skips missing assets when OPENCLAW_A2UI_SKIP_MISSING=1", async () => {
    await withA2uiFixture(async (dir) => {
      const previous = process.env.NSEMCLAW_A2UI_SKIP_MISSING;
      process.env.NSEMCLAW_A2UI_SKIP_MISSING = "1";
      try {
        await expect(
        ...
        if (previous === undefined) {
          delete process.env.NSEMCLAW_A2UI_SKIP_MISSING;
        } else {
          process.env.NSEMCLAW_A2UI_SKIP_MISSING = previous;
        }
      }
    });
  });
