 ...
 * 决策：是否应该使用子代理
 */
export function decideSubagentUsage(args: {
  ...
  try {
    ...
    const decision = integration.decideSubagentSpawn(context);
    
    // 4. 选择推荐模型
    const recommendedModel = selectModel(args.availableModels, decision.strategy);
    ...
    const reasoning = buildReasoning(complexity, args.currentLoad, decision.strategy);

    return {
      shouldSpawn: decision.shouldSpawn,
      strategy: decision.strategy,
      recommendedModel,
      estimatedTime,
      confidence: 0.7, // 可以从决策引擎获取
 ...
 * 批量决策多个任务
 */
export function decideBatchSubagentUsage(args: {
  tasks: BatchTask[];
  parentSessionKey: string;
  availableModels: string[];
  currentLoad: number;
