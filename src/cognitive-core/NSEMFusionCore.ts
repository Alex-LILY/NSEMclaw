...
import { DecisionModelEngine, getDecisionModelEngine } from "./decision/DecisionModelEngine.js";
import { SmartDecisionService, getSmartDecisionService } from "./decision/SmartDecisionService.js";
...
export class NSEMFusionCore extends EventEmitter {
  ...
  private decisionModelEngine?: DecisionModelEngine;
  private visionModelEngine?: VisionModelEngine;
  private smartDecisionService?: SmartDecisionService;
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      // 2. 初始化决策模型引擎 (Phi-4-mini)
      await this.initializeDecisionModel();

      // 3. 初始化视觉模型引擎 (Llava-Phi-3-mini，按需加载)
      await this.initializeVisionModel();
      ...
      // 8. 初始化内容路由器
      await this.initializeContentRouter();
  ...
  async shutdown(): Promise<void> {
    ...
    // 清理决策模型引擎
    if (this.decisionModelEngine) {
      await this.decisionModelEngine.unloadModel();
    }
    
    // 清理视觉模型引擎 (如果已加载)
    if (this.visionModelEngine) {
      await this.visionModelEngine.unloadModel();
    }
  ...
  getStatus(): FusionCoreStatus {
    ...
    return {
      ...
      vision: {
        available: this.visionModelEngine?.isModelAvailable() ?? false,
        loaded: this.visionModelEngine?.getStatus().loaded ?? false,
        idleTime: this.visionModelEngine?.getStatus().idleTime ?? 0,
      },
    };
  }
  ...
  /**
   * 分析图片内容
   * 使用本地 Llava-Phi-3-mini 模型，按需加载到 GPU
   * 
   * @param request 图片分析请求
   * @returns 分析结果
   * @throws 如果视觉模型未安装或分析失败
   */
  async analyzeImage(request: ImageAnalysisRequest): Promise<ImageAnalysisResult> {
    if (!this.visionModelEngine) {
      throw new Error("Vision model engine not initialized");
    }
    
    if (!this.visionModelEngine.isModelAvailable()) {
      throw new Error(
        "Vision model not installed. " +
        "Run 'npx nsemclaw-cli local-model vision' to install."
      );
    }
    
    return this.visionModelEngine.analyzeImage(request);
  }
  ...
  isVisionModelAvailable(): boolean {
    return this.visionModelEngine?.isModelAvailable() ?? false;
  }
   ...
   * 用于高级用法（直接控制加载/卸载）
   */
  getVisionEngine(): VisionModelEngine | undefined {
    return this.visionModelEngine;
  }
   ...
   * 4. 返回增强后的上下文供 LLM 使用
   * 
   * @param message 用户消息（包含文本和附件）
   * @returns 路由结果，包含视觉分析结果（如果需要）
   */
  async processUserMessage(message: UserMessage): Promise<ContentRoutingResult> {
    if (!this.initialized) {
      throw new Error("NSEMFusionCore not initialized");
    }

    if (!this.contentRouter) {
      throw new Error("Content router not initialized");
    }

    log.info(`Processing user message from ${message.userId}, ` +
             `attachments: ${message.attachments?.length || 0}`);

    // 使用内容路由器处理消息
    const result = await this.contentRouter.routeMessage(message);

    log.info(`Routing decision: ${result.decision.route}, ` +
             `task: ${result.decision.taskType || 'N/A'}, ` +
   ...
   * 不进行视觉分析，只做路由决策
   */
  routeUserMessageFast(message: UserMessage): { route: string; taskType?: string } {
    if (!this.contentRouter) {
      return { route: "llm" };
    }

    const decision = this.contentRouter.routeFast(message);
    return {
  ...
  private async initializeDecisionModel(): Promise<void> {
    log.info("Initializing decision model engine (Phi-4-mini)...");
    
    try {
      // 创建决策模型引擎
      this.decisionModelEngine = getDecisionModelEngine({
      ...
      // 预加载模型到 GPU
      await this.decisionModelEngine.loadModel();
      
      log.info("✅ Decision model engine initialized (Phi-4-mini loaded to GPU)");
    } catch (error) {
      log.warn("Failed to initialize decision model engine:", error as Record<string, unknown>);
      log.warn("Decision system will fall back to rule-based mode");
      // 不抛出错误，允许系统继续运行（决策系统有回退机制）
    }
  }

  private async initializeVisionModel(): Promise<void> {
    log.info("Initializing vision model engine (Llava-Phi-3-mini, on-demand loading)...");
    
    try {
      // 创建视觉模型引擎 (不立即加载，按需加载)
      this.visionModelEngine = getVisionModelEngine({
        gpuLayers: 999,
        contextLength: 4096,
        temperature: 0.2,
        maxTokens: 1024,
        idleTimeoutMs: 5 * 60 * 1000, // 5分钟空闲后卸载
      });
      
      // 检查模型是否可用
      if (this.visionModelEngine.isModelAvailable()) {
        log.info("✅ Vision model engine initialized (Llava-Phi-3-mini ready, on-demand load)");
      } else {
        log.info("⚠️  Vision model not installed. Run 'npx nsemclaw-cli local-model vision' to install");
      }
    } catch (error) {
      log.warn("Failed to initialize vision model engine:", error as Record<string, unknown>);
      // 不抛出错误，视觉模型是可选的
    }
  }
  ...
  private async initializeContentRouter(): Promise<void> {
    log.info("Initializing content router...");
    
    try {
      // 初始化智能决策服务
      this.smartDecisionService = getSmartDecisionService({
        enabled: true,
        engineWeight: 0.3,
        emotionWeight: 0.4,
      ...
      // 初始化内容路由器
      this.contentRouter = getContentRouter({
        decisionService: this.smartDecisionService,
        visionEngine: this.visionModelEngine,
      });

      log.info("✅ Content router initialized");
    } catch (error) {
      log.warn("Failed to initialize content router:", error as Record<string, unknown>);
      // 不抛出错误，允许系统继续运行
    }
  }
...
export type { UserMessage, ContentRoutingResult } from "./routing/index.js";
export type { ImageAnalysisRequest, ImageAnalysisResult } from "./vision/index.js";
