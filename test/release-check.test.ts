...
describe("collectAppcastSparkleVersionErrors", () => {
  ...
  it("requires lane-floor builds on and after lane-floor cutover", () => {
    const xml = `<rss><channel>${makeItem("2026.3.4", "202602270")}</channel></rss>`;

    expect(collectAppcastSparkleVersionErrors(xml)).toEqual([
      "appcast item '2026.3.4' has sparkle:version 202602270 below lane floor 2026022790.",
    ]);
  });

  it("accepts canonical stable lane builds on and after lane-floor cutover", () => {
    const xml = `<rss><channel>${makeItem("2026.3.4", "2026022790")}</channel></rss>`;
