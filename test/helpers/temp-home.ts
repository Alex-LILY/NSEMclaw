...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    nsemclawHome: process.env.NSEMCLAW_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("NSEMCLAW_HOME", snapshot.nsemclawHome);
  restoreKey("NSEMCLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked NSEMCLAW_HOME.
  delete process.env.NSEMCLAW_HOME;
  process.env.NSEMCLAW_STATE_DIR = path.join(base, ".nsemclaw");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "nsemclaw-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".nsemclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
