...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `nsemclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".nsemclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          NSEMCLAW_SKIP_PROVIDERS: "1",
          NSEMCLAW_SKIP_GMAIL_WATCHER: "1",
          NSEMCLAW_SKIP_CRON: "1",
          NSEMCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
          NSEMCLAW_SKIP_CANVAS_HOST: "1",
          NSEMCLAW_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
