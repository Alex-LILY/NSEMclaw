...
process.env.NSEMCLAW_PLUGIN_MANIFEST_CACHE_MS ??= "60000";
...
import type { NsemclawConfig } from "../src/config/config.js";
import type { OutboundSendDeps } from "../src/infra/outbound/deliver.js";
import { withIsolatedTestHome } from "./test-env.js";

// Set HOME/state isolation before importing any runtime Nsemclaw modules.
...
const pickSendFn = (id: ChannelId, deps?: OutboundSendDeps) => {
  switch (id) {
    case "discord":
      // @ts-expect-error - Dynamic access to optional property
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: NsemclawConfig) => {
    ...
    resolveAccount: (cfg: NsemclawConfig, accountId?: string | null) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") {
        return {};
    ...
    isConfigured: async (_account, cfg: NsemclawConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
