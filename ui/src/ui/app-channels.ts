...
export async function handleWhatsAppStart(host: NsemclawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: NsemclawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: NsemclawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: NsemclawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: NsemclawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: NsemclawApp): string {
...
function resolveGatewayHttpAuthHeader(host: NsemclawApp): string | null {
...
function buildGatewayHttpHeaders(host: NsemclawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: NsemclawApp) {
...
export function handleNostrProfileToggleAdvanced(host: NsemclawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: NsemclawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: NsemclawApp) {
  const state = host.nostrProfileFormState;
