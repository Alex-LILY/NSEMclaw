...
function renderSessionSummary(
  ...
  if (!usage) {
    return html`
      <div class="muted">${t("usage.session.noData")}</div>
    `;
  }
  ...
  if (session.modelProvider || session.providerOverride) {
    badges.push(`${t("usage.provider")}:${session.modelProvider ?? session.providerOverride}`);
  }
  if (session.model) {
    badges.push(`${t("usage.model")}:${session.model}`);
  }
  ...
  return html`
    ...
    <div class="session-summary-grid">
      <div class="session-summary-card">
        ...
        <div class="session-summary-value">${String(usage.messageCounts?.total ?? 0)}</div>
        <div class="session-summary-meta">${usage.messageCounts?.user ?? 0} ${t("usage.user")} · ${usage.messageCounts?.assistant ?? 0} ${t("usage.assistant")}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usage.overview.toolCalls")}</div>
        <div class="session-summary-value">${String(toolCallCount)}</div>
        <div class="session-summary-meta">${uniqueToolCount} ${t("usage.tools")}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usage.overview.errors")}</div>
        <div class="session-summary-value">${String(usage.messageCounts?.errors ?? 0)}</div>
        <div class="session-summary-meta">${usage.messageCounts?.toolResults ?? 0} ${t("usage.toolResults")}</div>
      </div>
      <div class="session-summary-card">
...
function renderSessionDetailPanel(
  ...
  const cursorIndicator = filteredUsage ? ` (${t("usage.filtered")})` : "";
  ...
  return html`
    <div class="card session-detail-panel">
      <div class="session-detail-header">
        ...
        <button class="session-close-btn" @click=${onClose} title=${t("usage.close")}>×</button>
      </div>
  ...
  return html`
    <div class="session-timeseries-compact">
      <div class="timeseries-header-row">
        <div class="card-title" style="font-size: 12px; color: var(--text);">${t("usage.usageOverTime")}</div>
        <div class="timeseries-controls">
          ${
            hasSelection
              ? html`
            <div class="chart-toggle small">
              <button class="toggle-btn active" @click=${() => onCursorRangeChange?.(null, null)}>${t("usage.reset")}</button>
            </div>
          `
              : nothing
          }
        ...
        >
          ...
          ${points.map((p, i) => {
            ...
            const tooltipLines = [
              ...
              `${formatTokens(val)} ${t("usage.tokens")}`,
            ];
            if (breakdownByType) {
              tooltipLines.push(`${t("usage.breakdown.out")} ${formatTokens(p.output)}`);
              tooltipLines.push(`${t("usage.breakdown.in")} ${formatTokens(p.input)}`);
              tooltipLines.push(`${t("usage.breakdown.cw")} ${formatTokens(p.cacheWrite)}`);
              tooltipLines.push(`${t("usage.breakdown.cr")} ${formatTokens(p.cacheRead)}`);
            }
      ...
      <div class="timeseries-summary">
        ${
          hasSelection
            ? html`
              <span style="color: var(--accent);">▶ ${t("usage.turns", { start: String(rangeStartIdx + 1), end: String(rangeEndIdx), total: String(points.length) })}</span> · 
            ...
            `
            : html`${points.length} ${t("usage.msgs")} · ${formatTokens(cumTokens)} · ${formatCost(cumCost)}`
        }
      </div>
      ${
        breakdownByType
          ? html`
              <div style="margin-top: 8px;">
                <div class="card-title" style="font-size: 12px; margin-bottom: 6px; color: var(--text);">${t("usage.tokensByType")}</div>
...
function renderContextPanel(
  ...
  if (usage && usage.totalTokens > 0) {
    ...
    if (inputTokens > 0) {
      contextPct = t("usage.context.percentage", {
  ...
  return html`
    <div class="context-details-panel">
      <div class="context-breakdown-header">
        <div class="card-title" style="font-size: 12px; color: var(--text);">${t("usage.context.title")}</div>
        ${
          hasMore
            ? html`<button class="context-expand-btn" @click=${onToggleExpanded}>
                ${showAll ? t("usage.context.collapse") : t("usage.context.expand")}
              </button>`
            : nothing
        }
      </div>
      <p class="context-weight-desc">
        ${contextPct || t("usage.context.base")}
      </p>
      <div class="context-stacked-bar">
        <div class="context-segment system" style="width: ${pct(systemTokens, totalContextTokens).toFixed(1)}%" title="${t("usage.context.system")}: ~${formatTokens(systemTokens)}"></div>
      ...
      <div class="context-legend">
        <span class="legend-item"><span class="legend-dot system"></span>${t("usage.context.systemShort")} ~${formatTokens(systemTokens)}</span>
        <span class="legend-item"><span class="legend-dot skills"></span>${t("usage.context.skillsShort")} ~${formatTokens(skillsTokens)}</span>
        <span class="legend-item"><span class="legend-dot tools"></span>${t("usage.context.toolsShort")} ~${formatTokens(toolsTokens)}</span>
        <span class="legend-item"><span class="legend-dot files"></span>${t("usage.context.filesShort")} ~${formatTokens(filesTokens)}</span>
      </div>
      <div class="context-total">${t("usage.total")}: ~${formatTokens(totalContextTokens)}</div>
            ...
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.context.skills")} (${skillsList.length})</div>
                    ...
                    ${
                      more > 0
                        ? html`<div class="context-breakdown-more">+${more} ${t("usage.more")}</div>`
                        : nothing
                    }
                  </div>
                `;
        ...
        ${
          toolsList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.context.tools")} (${toolsList.length})</div>
                    <div class="context-breakdown-list">
                      ${toolsTop.map(
                        (t) => html`
                          <div class="context-breakdown-item">
        ...
        ${
          filesList.length > 0
            ? (() => {
                ...
                return html`
                  <div class="context-breakdown-card">
                    <div class="context-breakdown-title">${t("usage.context.files")} (${filesList.length})</div>
                    <div class="context-breakdown-list">
                      ${filesTop.map(
                        (f) => html`
                          <div class="context-breakdown-item">
...
function renderSessionLogsCompact(
  ...
  if (loading) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">${t("usage.conversation.title")}</div>
        <div class="muted" style="padding: 20px; text-align: center">${t("common.loading")}</div>
      </div>
    `;
  }
  if (!logs || logs.length === 0) {
    return html`
      <div class="session-logs-compact">
        <div class="session-logs-header">${t("usage.conversation.title")}</div>
        <div class="muted" style="padding: 20px; text-align: center">${t("usage.conversation.noMessages")}</div>
      </div>
    `;
  }
  ...
  const displayedCount =
    hasActiveFilters || hasCursorFilter
      ? t("usage.conversation.filteredCount", {
          filtered: String(filteredEntries.length),
          total: String(logs.length),
          timeline: String(hasCursorFilter),
        })
      : `${logs.length}`;
  ...
  return html`
    <div class="session-logs-compact">
      <div class="session-logs-header">
        <span>${t("usage.conversation.title")} <span style="font-weight: normal; color: var(--muted);">(${displayedCount} ${t("usage.conversation.messages")})</span></span>
        <button class="btn btn-sm usage-action-btn usage-secondary-btn" @click=${onToggleExpandedAll}>
          ${expandedAll ? t("usage.conversation.collapseAll") : t("usage.conversation.expandAll")}
        </button>
      </div>
      <div class="usage-filters-inline" style="margin: 10px 12px;">
        ...
        >
          <option value="user" ?selected=${roleSelected.has("user")}>${t("usage.role.user")}</option>
          <option value="assistant" ?selected=${roleSelected.has("assistant")}>${t("usage.role.assistant")}</option>
          <option value="tool" ?selected=${roleSelected.has("tool")}>${t("usage.role.tool")}</option>
          <option value="toolResult" ?selected=${roleSelected.has("toolResult")}>${t("usage.role.toolResult")}</option>
        </select>
        <select
          multiple
          size="4"
        ...
        <label class="usage-filters-inline" style="gap: 6px;">
          ...
          />
          ${t("usage.conversation.hasTools")}
        </label>
        <input
          type="text"
          placeholder=${t("usage.conversation.search")}
          .value=${filters.query}
      ...
      <div class="session-logs-list">
        ...
        ${
          filteredEntries.length === 0
            ? html`
                <div class="muted" style="padding: 12px">${t("usage.conversation.noMatch")}</div>
              `
            : nothing
        }
      </div>
