...
function buildPeakErrorHours(sessions: UsageSessionEntry[], timeZone: "local" | "utc") {
  ...
  return hourMsgs
    ...
    .map((entry) => ({
      ...
      sub: `${Math.round(entry.errors)} ${t("usage.errors")} · ${Math.round(entry.msgs)} ${t("usage.msgs")}`,
    }));
}
      ...
      <div class="card usage-mosaic">
        <div class="usage-mosaic-header">
          <div>
            ...
            <div class="usage-mosaic-sub">${t("usage.mosaic.subtitle")}</div>
          </div>
      ...
      <div class="usage-mosaic-header">
        <div>
          ...
          <div class="usage-mosaic-sub">
            ${t("usage.mosaic.timeZone", { zone: timeZone === "utc" ? t("usage.utc") : t("usage.local") })}
          </div>
        </div>
        ...
        <div class="usage-mosaic-section">
          ...
          <div class="usage-hour-grid">
            ${stats.hourTotals.map((value, hour) => {
              ...
              const title = `${hour}:00 · ${formatTokens(value)} ${t("usage.tokens")}`;
          ...
          <div class="usage-hour-labels">
            <span>${t("usage.hour.midnight")}</span>
            <span>${t("usage.hour.4am")}</span>
            <span>${t("usage.hour.8am")}</span>
            <span>${t("usage.hour.noon")}</span>
            <span>${t("usage.hour.4pm")}</span>
            <span>${t("usage.hour.8pm")}</span>
          </div>
