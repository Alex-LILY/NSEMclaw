...
const SECURITY_OPTIONS: Array<{ value: ExecSecurity; label: string }> = [
  { value: "deny", label: t("execApprovals.security.deny") },
  { value: "allowlist", label: t("execApprovals.security.allowlist") },
  { value: "full", label: t("execApprovals.security.full") },
];

const ASK_OPTIONS: Array<{ value: ExecAsk; label: string }> = [
  { value: "off", label: t("execApprovals.ask.off") },
  { value: "on-miss", label: t("execApprovals.ask.onMiss") },
  { value: "always", label: t("execApprovals.ask.always") },
];
      ...
      ${
        !ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            <div class="muted">${t("execApprovals.loadPrompt")}</div>
            <button class="btn" ?disabled=${state.loading || !targetReady} @click=${state.onLoad}>
              ${state.loading ? t("common.loading") : t("execApprovals.load")}
            </button>
          </div>`
          : html`
    ...
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${t("execApprovals.target.description")}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
            ...
            >
              <option value="gateway" ?selected=${state.target === "gateway"}>${t("execApprovals.target.gateway")}</option>
              <option value="node" ?selected=${state.target === "node"}>${t("execApprovals.target.node")}</option>
            </select>
          </label>
          ${
            state.target === "node"
              ? html`
                <label class="field">
                  <span>${t("execApprovals.target.nodeLabel")}</span>
                  <select
                  ...
                  >
                    <option value="" ?selected=${nodeValue === ""}>${t("execApprovals.target.selectNode")}</option>
    ...
    <div class="list" style="margin-top: 16px;">
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${isDefaults ? t("execApprovals.security.defaultDescription") : t("execApprovals.security.agentDescription", { default: defaults.security })}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
      ...
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${isDefaults ? t("execApprovals.ask.defaultDescription") : t("execApprovals.ask.agentDescription", { default: defaults.ask })}
          </div>
        </div>
        <div class="list-meta">
          <label class="field">
      ...
      <div class="list-item">
        <div class="list-main">
          ...
          <div class="list-sub">
            ${
              isDefaults
                ? t("execApprovals.askFallback.defaultDescription")
                : t("execApprovals.askFallback.agentDescription", { default: defaults.askFallback })
            }
          </div>
        </div>
        <div class="list-meta">
      ...
      <div class="list-item">
        <div class="list-main">
          <div class="list-title">${t("execApprovals.autoAllowSkills.title")}</div>
          <div class="list-sub">
            ${
              isDefaults
                ? t("execApprovals.autoAllowSkills.defaultDescription")
                : autoIsDefault
                  ? t("execApprovals.autoAllowSkills.usingDefault", {
                      value: defaults.autoAllowSkills ? t("common.on") : t("common.off"),
                    })
                  : t("execApprovals.autoAllowSkills.override", {
        ...
        <div class="list-meta">
          ...
          ${
            !isDefaults && !autoIsDefault
              ...
              >
                ${t("execApprovals.useDefaultLabel")}
              </button>`
              : nothing
          }
        </div>
  ...
  return html`
    <div class="row" style="margin-top: 18px; justify-content: space-between;">
      <div>
        ...
        <div class="card-sub">${t("execApprovals.allowlist.description")}</div>
      </div>
      <button
        class="btn btn--sm"
        ?disabled=${state.disabled}
    ...
    <div class="list" style="margin-top: 12px;">
      ${
        entries.length === 0
          ? html`
              <div class="muted">${t("execApprovals.allowlist.noEntries")}</div>
            `
...
function renderAllowlistEntry(
  ...
  const lastUsed = entry.lastUsedAt
    ...
    : t("execApprovals.allowlist.never");
  const lastCommand = entry.lastUsedCommand ? clampText(entry.lastUsedCommand, 120) : null;
  const lastPath = entry.lastResolvedPath ? clampText(entry.lastResolvedPath, 120) : null;
  return html`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${entry.pattern?.trim() ? entry.pattern : t("execApprovals.allowlist.newPattern")}</div>
        <div class="list-sub">${t("execApprovals.allowlist.lastUsed")}: ${lastUsed}</div>
