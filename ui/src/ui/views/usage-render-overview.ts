        ...
        selectedDays.length > 0
          ? html`
            <div class="filter-chip">
              ...
              <button class="filter-chip-remove" @click=${onClearDays} title=${t("usage.filters.remove")}>×</button>
            </div>
          `
          : nothing
      }
        ...
        selectedHours.length > 0
          ? html`
            <div class="filter-chip">
              ...
              <button class="filter-chip-remove" @click=${onClearHours} title=${t("usage.filters.remove")}>×</button>
            </div>
          `
          : nothing
      }
        ...
        selectedSessions.length > 0
          ? html`
            <div class="filter-chip" title="${sessionsFullName}">
              ...
              <button class="filter-chip-remove" @click=${onClearSessions} title=${t("usage.filters.remove")}>×</button>
            </div>
          `
          : nothing
      }
  ...
  if (!daily.length) {
    return html`
      <div class="daily-chart-compact">
        <div class="sessions-panel-title">${t("usage.daily.title")}</div>
  ...
  return html`
    <div class="daily-chart-compact">
      <div class="daily-chart-header">
        ...
        <div class="card-title">${t("usage.daily.titleWithType", { type: isTokenMode ? t("usage.tokens") : t("usage.cost") })}</div>
      </div>
            ...
            return html`
              ...
              >
                ...
                <div class="daily-bar-tooltip">
                  ...
                  ${formatTokens(d.totalTokens)} ${t("usage.tokens")}<br />
    ...
    <div class="cost-breakdown cost-breakdown-compact">
      ...
      <div class="cost-breakdown-bar">
        ...
        <div class="cost-segment cache-write" style="width: ${(isTokenMode ? tokenPcts.cacheWrite : breakdown.cacheWrite.pct).toFixed(1)}%"
          title="${t("usage.breakdown.cacheWriteTitle")}: ${isTokenMode ? formatTokens(totals.cacheWrite) : formatCost(breakdown.cacheWrite.cost)}"></div>
        <div class="cost-segment cache-read" style="width: ${(isTokenMode ? tokenPcts.cacheRead : breakdown.cacheRead.pct).toFixed(1)}%"
          title="${t("usage.breakdown.cacheReadTitle")}: ${isTokenMode ? formatTokens(totals.cacheRead) : formatCost(breakdown.cacheRead.cost)}"></div>
      </div>
      ...
      <div class="cost-breakdown-total">
        ${t("usage.total")}: ${isTokenMode ? formatTokens(totals.totalTokens) : formatCost(totals.totalCost)}
      </div>
    </div>
  `;
}
...
function renderUsageInsights(
  ...
  const throughputLabel =
    stats.throughputTokensPerMin !== undefined
      ? `${formatTokens(Math.round(stats.throughputTokensPerMin))} ${t("usage.throughputUnit")}`
      : "—";
  const throughputCostLabel =
    stats.throughputCostPerMin !== undefined
      ? `${formatCost(stats.throughputCostPerMin, 4)} / ${t("common.min")}`
      : "—";
  ...
  const errorDays = aggregates.daily
    ...
    .map((day) => {
      ...
      return {
        ...
        sub: `${day.errors} ${t("usage.errors")} · ${day.messages} ${t("usage.msgs")} · ${formatTokens(day.tokens)}`,
        rate,
      };
    })
      ...
      <div class="usage-summary-grid">
        ...
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ...
            <span class="usage-summary-hint" title=${showCostHint ? t("usage.overview.tokensHint") : t("usage.overview.tokensHint")}>?</span>
          </div>
          <div class="usage-summary-value">${formatTokens(avgTokens)}</div>
          <div class="usage-summary-sub">${t("usage.across")} ${aggregates.messages.total || 0} ${t("usage.messages")}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usage.overview.avgCost")}
            <span class="usage-summary-hint" title=${showCostHint ? t("usage.overview.costHintWithWarning") : t("usage.overview.costHint")}>?</span>
          </div>
        ...
        <div class="usage-summary-card">
          ...
          <div class="usage-summary-sub">${t("usage.of")} ${totalSessions} ${t("usage.inRange")}</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usage.overview.throughput")}
            <span class="usage-summary-hint" title=${t("usage.overview.throughputHint")}>?</span>
          </div>
        ...
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ...
            <span class="usage-summary-hint" title=${t("usage.overview.errorRateHint")}>?</span>
          </div>
          <div class="usage-summary-value ${errorRatePct > 5 ? "bad" : errorRatePct > 1 ? "warn" : "good"}">${errorRatePct.toFixed(2)}%</div>
          <div class="usage-summary-sub">
            ${aggregates.messages.errors} ${t("usage.errors")} · ${avgDurationLabel} ${t("usage.avgSession")}
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            ${t("usage.overview.cacheHitRate")}
            <span class="usage-summary-hint" title=${t("usage.overview.cacheHitRateHint")}>?</span>
          </div>
...
function renderSessionsCard(
  ...
  const buildSessionMeta = (s: UsageSessionEntry): string[] => {
    ...
    if (showColumn("channel") && s.channel) {
      parts.push(`${t("usage.channel")}:${s.channel}`);
    }
    if (showColumn("agent") && s.agentId) {
      parts.push(`${t("usage.agent")}:${s.agentId}`);
    }
    if (showColumn("provider") && (s.modelProvider || s.providerOverride)) {
      parts.push(`${t("usage.provider")}:${s.modelProvider ?? s.providerOverride}`);
    }
    if (showColumn("model") && s.model) {
      parts.push(`${t("usage.model")}:${s.model}`);
    }
    if (showColumn("messages") && s.usage?.messageCounts) {
      parts.push(`${t("usage.msgs")}:${s.usage.messageCounts.total}`);
    }
    if (showColumn("tools") && s.usage?.toolUsage) {
      parts.push(`${t("usage.tools")}:${s.usage.toolUsage.totalCalls}`);
    }
    if (showColumn("errors") && s.usage?.messageCounts) {
      parts.push(`${t("usage.errors")}:${s.usage.messageCounts.errors}`);
    }
    if (showColumn("duration") && s.usage?.durationMs) {
      parts.push(
        `${t("usage.dur")}:${formatDurationCompact(s.usage.durationMs, { spaced: true }) ?? "—"}`,
      );
    }
    return parts;
  };
    ...
    <div class="card sessions-card">
      <div class="sessions-card-header">
        ...
        <div class="sessions-card-count">
          ${t("usage.sessions.shown", { shown: String(sessions.length), total: totalSessions !== sessions.length ? String(totalSessions) : "" })}
        </div>
      </div>
      <div class="sessions-card-meta">
        <div class="sessions-card-stats">
          <span>${isTokenMode ? formatTokens(avgValue) : formatCost(avgValue)} ${t("usage.avg")}</span>
          <span>${totalErrors} ${t("usage.errors")}</span>
        </div>
        <div class="chart-toggle small">
          ...
          >
            ${t("usage.sessions.recentlyViewed")}
          </button>
        </div>
      ...
      ${
        sessionsTab === "recent"
          ...
          : sessions.length === 0
            ? html`
                <div class="muted" style="padding: 20px; text-align: center">${t("usage.sessions.noSessions")}</div>
              `
            : html`
