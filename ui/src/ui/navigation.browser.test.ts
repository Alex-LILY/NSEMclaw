...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/nsemclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/nsemclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/nsemclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__NSEMCLAW_CONTROL_UI_BASE_PATH__ = "/nsemclaw";
    const app = mountApp("/nsemclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/nsemclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/nsemclaw/sessions");
  });
