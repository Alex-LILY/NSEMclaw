...
import type { NsemclawPluginApi, NsemclawPluginService } from "nsemclaw/plugin-sdk";
...
function normalizeDenyList(cfg: NsemclawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: NsemclawPluginApi["config"]): string[] {
...
export default function register(api: NsemclawPluginApi) {
  ...
  const timerService: NsemclawPluginService = {
