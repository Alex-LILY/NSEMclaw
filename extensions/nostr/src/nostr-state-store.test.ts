...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.NSEMCLAW_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nsemclaw-nostr-"));
  process.env.NSEMCLAW_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        ...
        return path.join(resolveHome(), ".nsemclaw");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.NSEMCLAW_STATE_DIR = previous;
    }
