...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["nsemclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["nsemclaw", "pollId"]) ??
    readNestedString(value, ["nsemclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "nsemclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "nsemclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          nsemclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "nsemclaw poll vote",
          displayText: "Vote recorded",
          value: { nsemclawPollId: pollId, pollId },
        },
      },
    ],
  };
