...
vi.mock("nsemclaw/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("nsemclaw/plugin-sdk")>("nsemclaw/plugin-sdk");
  return {
    ...actual,
...
import type { NsemclawPluginServiceContext } from "nsemclaw/plugin-sdk";
import { emitDiagnosticEvent } from "nsemclaw/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/nsemclaw-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): NsemclawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("nsemclaw.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("nsemclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("nsemclaw.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("nsemclaw.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("nsemclaw.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("nsemclaw.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("nsemclaw.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("nsemclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("nsemclaw.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("nsemclaw.webhook.processed");
    expect(spanNames).toContain("nsemclaw.message.processed");
    expect(spanNames).toContain("nsemclaw.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["nsemclaw.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("nsemclaw.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "nsemclaw.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["nsemclaw.reason"]).toBe("string");
    expect(String(attrs?.["nsemclaw.reason"])).not.toContain(
