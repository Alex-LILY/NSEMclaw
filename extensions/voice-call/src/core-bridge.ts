...
function resolveNsemclawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.NSEMCLAW_ROOT?.trim();
  if (override) {
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveNsemclawRoot(), "dist", "extensionAPI.js");
