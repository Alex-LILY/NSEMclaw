/**
 * Twitch channel plugin for Nsemclaw.
 *
...
import { buildChannelConfigSchema } from "nsemclaw/plugin-sdk";
 ...
 * for Nsemclaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: NsemclawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: NsemclawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: NsemclawConfig): boolean => {
