...
function resolveGatewayPort(cfg: NsemclawPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.NSEMCLAW_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: NsemclawPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: NsemclawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Nsemclaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Nsemclaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",
