...
import { withTempDownloadPath, type ClawdbotConfig } from "nsemclaw/plugin-sdk";
...
export async function downloadImageFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "nsemclaw-feishu-img-",
...
export async function downloadMessageResourceFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "nsemclaw-feishu-resource-",
...
export async function sendMediaFeishu(params: {
  ...
  if (mediaBuffer) {
    ...
    // @ts-expect-error loadWebMedia may not exist in runtime type
