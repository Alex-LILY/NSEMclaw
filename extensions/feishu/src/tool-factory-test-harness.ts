import type { AnyAgentTool, NsemclawPluginApi } from "nsemclaw/plugin-sdk";
...
export function createToolFactoryHarness(cfg: NsemclawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<NsemclawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
