...
This plan defines how Nsemclaw should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- Nsemclaw core owns ACP control plane concerns
...
Nsemclaw should not reimplement ACP transport internals in core.
Nsemclaw should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in Nsemclaw, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under Nsemclaw state dir
...
- ACP event stream is normalized to Nsemclaw reply chunks
...
For the first production backend (`extensions/acpx`), Nsemclaw and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses Nsemclaw `sessionKey`)
...
- Nsemclaw consumes ndjson events from `acpx --format json --json-strict`
