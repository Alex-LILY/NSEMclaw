...
read_when:
  - Designing workspace memory (~/.nsemclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Nsemclaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.nsemclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.nsemclaw/workspace` as canonical human-readable memory.
...
```
~/.nsemclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Nsemclaw**, but keep a separable core library.

### Why integrate into Nsemclaw?

- Nsemclaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `nsemclaw memory recall "…" --k 25 --since 30d`
  - `nsemclaw memory reflect --since 7d`
...
Pragmatic take for `~/.nsemclaw/workspace`:
