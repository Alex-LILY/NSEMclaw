...
- Publish as `nsemclaw/plugin-sdk` (or export from core under `nsemclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `NsemclawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: NsemclawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: NsemclawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: NsemclawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: NsemclawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: NsemclawConfig): string;
  };
};
```
...
- Introduce `nsemclaw/plugin-sdk`.
- Add `api.runtime` to `NsemclawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `nsemclawRuntime: ">=2026.2.0"`).
