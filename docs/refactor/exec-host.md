...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.nsemclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.nsemclaw/node.json` (node id + pairing token).
...
Path: `~/.nsemclaw/exec-approvals.json`
...
- Unix socket at `~/.nsemclaw/exec-approvals.sock` (0600).
