...
Matrix is an open, decentralized messaging protocol. Nsemclaw connects as a Matrix **user**
...
```bash
nsemclaw plugins install @nsemclaw/matrix
```

Local checkout (when running from a git repo):

```bash
nsemclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `nsemclaw plugins install @nsemclaw/matrix`
   - From a local checkout: `nsemclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Nsemclaw calls the same
     login endpoint, stores the access token in `~/.nsemclaw/credentials/matrix/credentials.json`,
...
- On first connection, Nsemclaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Nsemclaw logs a warning.
...
`~/.nsemclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `nsemclaw pairing list matrix`
  - `nsemclaw pairing approve matrix <CODE>`
...
- On startup, Nsemclaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
