...
```bash
nsemclaw plugins install @nsemclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
nsemclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Nsemclaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `nsemclaw pairing list nextcloud-talk`
  - `nsemclaw pairing approve nextcloud-talk <CODE>`
