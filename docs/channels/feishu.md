...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Nsemclaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
nsemclaw plugins install @nsemclaw/feishu
```

Local checkout (when running from a git repo):

```bash
nsemclaw plugins install ./extensions/feishu
```
...
If you just installed Nsemclaw, run the wizard:
...
2. Configuring app credentials in Nsemclaw
...
1. You already ran `nsemclaw channels add` for Feishu
2. The gateway is running (`nsemclaw gateway status`)
...
```bash
nsemclaw pairing approve feishu <CODE>
```
...
2. Run `nsemclaw logs --follow` and look for `chat_id`
...
2. Run `nsemclaw logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `nsemclaw gateway status`  | Show gateway status           |
| `nsemclaw gateway install` | Install/start gateway service |
| `nsemclaw gateway stop`    | Stop gateway service          |
| `nsemclaw gateway restart` | Restart gateway service       |
| `nsemclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `nsemclaw logs --follow`
...
5. Ensure the gateway is running: `nsemclaw gateway status`
6. Check logs: `nsemclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.nsemclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.nsemclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
