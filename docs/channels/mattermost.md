---
summary: "Mattermost bot setup and Nsemclaw config"
read_when:
...
```bash
nsemclaw plugins install @nsemclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
nsemclaw plugins install ./extensions/mattermost
```
...
4. Configure Nsemclaw and start the gateway.
...
- Approve via:
  - `nsemclaw pairing list mattermost`
  - `nsemclaw pairing approve mattermost <CODE>`
...
Use these target formats with `nsemclaw message send` or cron/webhooks:
