...
read_when:
  - Setting up Twitch chat integration for Nsemclaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Nsemclaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
nsemclaw plugins install @nsemclaw/twitch
```

Local checkout (when running from a git repo):

```bash
nsemclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `NSEMCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "nsemclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use NSEMCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
NSEMCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
