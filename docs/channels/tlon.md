...
Tlon is a decentralized messenger built on Urbit. Nsemclaw connects to your Urbit ship and can
...
```bash
nsemclaw plugins install @nsemclaw/tlon
```

Local checkout (when running from a git repo):

```bash
nsemclaw plugins install ./extensions/tlon
```
...
By default, Nsemclaw blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `nsemclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Nsemclaw replies in-thread.
