---
summary: "CLI reference for `nsemclaw secrets` (reload, audit, configure, apply)"
read_when:
...
Use `nsemclaw secrets` to migrate credentials from plaintext to SecretRefs and keep the active secrets runtime healthy.
...
```bash
nsemclaw secrets audit
nsemclaw secrets audit --check
nsemclaw secrets audit --json
```
...
```bash
nsemclaw secrets configure
nsemclaw secrets configure --plan-out /tmp/nsemclaw-secrets-plan.json
nsemclaw secrets configure --apply --yes
nsemclaw secrets configure --providers-only
nsemclaw secrets configure --skip-provider-setup
nsemclaw secrets configure --json
```
...
- `configure` targets secret-bearing fields in `nsemclaw.json`.
...
nsemclaw secrets apply --from /tmp/nsemclaw-secrets-plan.json
nsemclaw secrets apply --from /tmp/nsemclaw-secrets-plan.json --dry-run
nsemclaw secrets apply --from /tmp/nsemclaw-secrets-plan.json --json
```
...
- `nsemclaw.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.nsemclaw/.env` known secret keys whose values were migrated
