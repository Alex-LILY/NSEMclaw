---
summary: "CLI reference for `nsemclaw onboard` (interactive onboarding wizard)"
read_when:
...
```bash
nsemclaw onboard
nsemclaw onboard --flow quickstart
nsemclaw onboard --flow manual
nsemclaw onboard --mode remote --remote-url ws://gateway-host:18789
```
...
- Fastest first chat: `nsemclaw dashboard` (Control UI, no channel setup).
