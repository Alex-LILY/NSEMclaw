...
- `/usage cost` in chats: local cost summary aggregated from Nsemclaw session logs.
- CLI: `nsemclaw status --usage` prints a full per-provider breakdown.
- CLI: `nsemclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
