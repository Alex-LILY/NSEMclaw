...
- When given a `sessionId`, Nsemclaw resolves it to the corresponding session key (missing ids error).
...
- After the primary run completes, Nsemclaw runs a **reply-back loop**:
  - Round 2+ alternates between requester and target agents.
  - Reply exactly `REPLY_SKIP` to stop the ping‑pong.
  - Max turns is `session.agentToAgent.maxPingPongTurns` (0–5, default 5).
- Once the loop ends, Nsemclaw runs the **agent‑to‑agent announce step** (target agent only):
...
- After completion, Nsemclaw runs a sub-agent **announce step** and posts the result to the requester chat channel.
...
- When a session is sandboxed and `sessionToolsVisibility="spawned"`, Nsemclaw clamps visibility to `tree` even if you set `tools.sessions.visibility="all"`.
