...
```bash
nsemclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `nsemclaw tui --deliver`
...
- Check the Gateway logs: `nsemclaw logs --follow`.
- Confirm the agent can run: `nsemclaw status` and `nsemclaw models status`.
...
- No agents in picker: check `nsemclaw agents list` and your routing config.
