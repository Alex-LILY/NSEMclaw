...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/nsemclaw`)
...
If the page fails to load, start the Gateway first: `nsemclaw gateway`.
...
you revoke it with `nsemclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.nsemclaw/nsemclaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop nsemclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Nsemclaw
...
```bash
nsemclaw gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
Nsemclaw **blocks** Control UI connections without device identity.
...
```bash
NSEMCLAW_CONTROL_UI_BASE_PATH=/nsemclaw/ pnpm ui:build
```
