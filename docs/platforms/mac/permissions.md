...
- Same path: run the app from a fixed location (for Nsemclaw, `dist/Nsemclaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.nsemclaw.mac
sudo tccutil reset ScreenCapture ai.nsemclaw.mac
...
Workaround: move files into the Nsemclaw workspace (`~/.nsemclaw/workspace`) if you want to avoid per-folder grants.
