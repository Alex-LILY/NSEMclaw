...
service via the external `nsemclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.nsemclaw.gateway`
  (or `ai.nsemclaw.<profile>` when using `--profile`/`NSEMCLAW_PROFILE`; legacy `com.nsemclaw.*` is supported).
