---
summary: "Nsemclaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Nsemclaw on a Raspberry Pi
  - Running Nsemclaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Nsemclaw on Raspberry Pi

## Goal

Run a persistent, always-on Nsemclaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Nsemclaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled nsemclaw

# Enable if not
sudo systemctl enable nsemclaw

# Start on boot
sudo systemctl start nsemclaw
```
...
```bash
# Check logs
journalctl -u nsemclaw --no-pager -n 100

# Common fix: rebuild
cd ~/nsemclaw  # if using hackable install
npm run build
sudo systemctl restart nsemclaw
```
