---
summary: "Nsemclaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Nsemclaw on DigitalOcean
  - Looking for cheap VPS hosting for Nsemclaw
title: "DigitalOcean"
---

# Nsemclaw on DigitalOcean

## Goal

Run a persistent Nsemclaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status nsemclaw-gateway.service

# View logs
journalctl --user -u nsemclaw-gateway.service -f
```
...
```bash
nsemclaw config set gateway.bind tailnet
nsemclaw gateway restart
```
...
```bash
nsemclaw channels login whatsapp
# Scan QR code
```
...
- `~/.nsemclaw/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u nsemclaw --no-pager -n 50
```
