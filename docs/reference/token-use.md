---
summary: "How Nsemclaw builds prompt context and reports token usage + costs"
read_when:
...
Nsemclaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Nsemclaw assembles its own system prompt on every run. It includes:
...
For images, Nsemclaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Nsemclaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `nsemclaw status --usage` and `nsemclaw channels list` show
...
`cacheWrite`. If pricing is missing, Nsemclaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Nsemclaw can
...
Anthropic's 1M context window is currently beta-gated. Nsemclaw can inject the
...
Nsemclaw skips the `context-1m-*` beta header because Anthropic currently
