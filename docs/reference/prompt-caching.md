...
- With Anthropic API-key auth profiles, Nsemclaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Nsemclaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Nsemclaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.nsemclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$NSEMCLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `NSEMCLAW_CACHE_TRACE=1` enables cache tracing.
- `NSEMCLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `NSEMCLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `NSEMCLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `NSEMCLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
