...
Nsemclaw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Nsemclaw owns lifecycle when `channels.signal.autoStart=true`.
...
- Nsemclaw spawns `imsg rpc` as a child process (legacy iMessage integration).
