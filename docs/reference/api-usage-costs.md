...
Nsemclaw features that can generate provider usage or paid API calls.
...
- `nsemclaw status --usage` and `nsemclaw channels list` show provider **usage windows**
...
Nsemclaw can pick up credentials from:
...
`nsemclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
