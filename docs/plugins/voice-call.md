...
read_when:
  - You want to place an outbound voice call from Nsemclaw
...
Voice calls for Nsemclaw via a plugin. Supports outbound notifications and
...
- Use `nsemclaw voicecall ...` or the `voice_call` tool
...
```bash
nsemclaw plugins install ./extensions/voice-call
...
```bash
nsemclaw voicecall call --to "+15555550123" --message "Hello from Nsemclaw"
nsemclaw voicecall continue --call-id <id> --message "Any questions?"
nsemclaw voicecall speak --call-id <id> --message "One moment"
nsemclaw voicecall end --call-id <id>
nsemclaw voicecall status --call-id <id>
nsemclaw voicecall tail
