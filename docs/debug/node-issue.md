...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Nsemclaw
...
Running Nsemclaw via Node with `tsx` fails at startup with:

```
[nsemclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `nsemclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch nsemclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node nsemclaw.mjs status` works on Node 25.
