...
- During heartbeat runs, Nsemclaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `nsemclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Nsemclaw skips the heartbeat run entirely (no model call).
...
```bash
nsemclaw system event --text "Check for urgent follow-ups" --mode now
```
