...
Nsemclaw supports additive secret references so credentials do not need to be stored as plaintext in config files.
...
When onboarding runs in interactive mode and you choose secret reference storage, Nsemclaw performs a fast preflight check before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Nsemclaw validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Nsemclaw QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/nsemclaw"],
...
When reload-time activation fails after a healthy state, Nsemclaw enters degraded secrets state.
...
```bash
nsemclaw secrets audit --check
nsemclaw secrets configure
nsemclaw secrets audit --check
```
...
- plaintext values at rest (`nsemclaw.json`, `auth-profiles.json`, `.env`)
- unresolved refs
...
- lets you select secret-bearing fields in `nsemclaw.json`
...
- `nsemclaw secrets configure --providers-only`
- `nsemclaw secrets configure --skip-provider-setup`
...
```bash
nsemclaw secrets apply --from /tmp/nsemclaw-secrets-plan.json
nsemclaw secrets apply --from /tmp/nsemclaw-secrets-plan.json --dry-run
```
...
Nsemclaw intentionally does **not** write rollback backups that contain pre-migration plaintext secret values.
...
For static credentials, Nsemclaw runtime no longer depends on plaintext `auth.json`.
