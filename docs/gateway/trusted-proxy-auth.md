...
read_when:
  - Running Nsemclaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Nsemclaw
...
- You run Nsemclaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Nsemclaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Nsemclaw extracts the user identity from the configured header
...
- Nsemclaw can stay on loopback HTTP behind the proxy.
...
If Nsemclaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://nsemclaw.example.com
    to: http://nsemclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
nsemclaw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy nsemclaw:18789 {
...
location / {
    ...
    proxy_pass http://nsemclaw:18789;
...
`nsemclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Nsemclaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `nsemclaw security audit` and review findings
