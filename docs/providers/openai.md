---
summary: "Use OpenAI via API keys or Codex subscription in Nsemclaw"
read_when:
  - You want to use OpenAI models in Nsemclaw
...
```bash
nsemclaw onboard --auth-choice openai-api-key
# or non-interactive
nsemclaw onboard --openai-api-key "$OPENAI_API_KEY"
```
...
nsemclaw onboard --auth-choice openai-codex
...
Nsemclaw uses `pi-ai` for model streaming. For `openai-codex/*` models you can set
...
`baseUrl` on `api.openai.com`), Nsemclaw now auto-enables OpenAI server-side
