---
summary: "Use MiniMax M2.1 in Nsemclaw"
read_when:
  - You want MiniMax models in Nsemclaw
...
```bash
nsemclaw plugins enable minimax-portal-auth  # skip if already loaded.
nsemclaw gateway restart  # restart if gateway is already running
nsemclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/nsemclaw/nsemclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `nsemclaw.json`:
...
- Use `nsemclaw models list` and `nsemclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `nsemclaw configure` and selecting **MiniMax M2.1**, or
