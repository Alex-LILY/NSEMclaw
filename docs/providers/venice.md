---
summary: "Use Venice AI privacy-focused models in Nsemclaw"
read_when:
  - You want privacy-focused inference in Nsemclaw
...
```bash
nsemclaw onboard --auth-choice venice-api-key
```
...
```bash
nsemclaw agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, Nsemclaw shows all available Venice models. Pick based on your needs:
...
```bash
nsemclaw models set venice/claude-opus-45
nsemclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
nsemclaw models list | grep venice
```

## Configure via `nsemclaw configure`

1. Run `nsemclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Nsemclaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
nsemclaw agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
nsemclaw agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
nsemclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
nsemclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
nsemclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `nsemclaw models list` to see currently available models. Some models may be temporarily offline.
