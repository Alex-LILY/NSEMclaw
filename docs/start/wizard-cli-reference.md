...
read_when:
  - You need detailed behavior for nsemclaw onboard
...
This page is the full reference for `nsemclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.nsemclaw/nsemclaw.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `nsemclaw doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `nsemclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `nsemclaw health`.
    - `nsemclaw status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.nsemclaw/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.nsemclaw/agents/<agentId>/agent/auth-profiles.json`
...
`~/.nsemclaw/credentials/oauth.json` (or `$NSEMCLAW_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`nsemclaw onboard`](/cli/onboard)
