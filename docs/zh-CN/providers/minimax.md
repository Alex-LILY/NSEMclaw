---
read_when:
  - 你想在 Nsemclaw 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 Nsemclaw 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
nsemclaw plugins enable minimax-portal-auth  # 如果已加载则跳过
nsemclaw gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/nsemclaw/nsemclaw/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `nsemclaw models list` 和 `nsemclaw models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `nsemclaw configure` 并选择 **MiniMax M2.1**，或
