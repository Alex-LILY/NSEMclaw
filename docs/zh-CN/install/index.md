...
read_when:
  - 安装 Nsemclaw
  - 你想从 GitHub 安装
summary: 安装 Nsemclaw（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
pnpm approve-builds -g                # 批准 nsemclaw、node-llama-cpp、sharp 等
pnpm add -g nsemclaw@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm nsemclaw ...` 运行仓库命令。
...
- 运行新手引导：`nsemclaw onboard --install-daemon`
- 快速检查：`nsemclaw doctor`
- 检查 Gateway 网关健康状态：`nsemclaw status` + `nsemclaw health`
- 打开仪表板：`nsemclaw dashboard`
...
```bash
# 显式 npm
curl -fsSL https://nsemclaw.ai/install.sh | bash -s -- --install-method npm
...
- `NSEMCLAW_GIT_DIR=...`
- `NSEMCLAW_GIT_UPDATE=0|1`
- `NSEMCLAW_NO_PROMPT=1`
- `NSEMCLAW_DRY_RUN=1`
- `NSEMCLAW_NO_ONBOARD=1`
- `SHARP_IGNORE_GLOBAL_LIBVIPS=0|1`（默认：`1`；避免 `sharp` 针对系统 libvips 构建）

## 故障排除：找不到 `nsemclaw`（PATH）
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `nsemclaw`）。
