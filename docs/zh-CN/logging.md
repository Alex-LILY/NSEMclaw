...
你可以在 `~/.nsemclaw/nsemclaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.nsemclaw/nsemclaw.json` 的 `logging` 下。
...
- Nsemclaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `nsemclaw plugins enable diagnostics-otel` 启用插件。
...
- `nsemclaw.tokens`（计数器，属性：`nsemclaw.token`、`nsemclaw.channel`、`nsemclaw.provider`、`nsemclaw.model`）
- `nsemclaw.cost.usd`（计数器，属性：`nsemclaw.channel`、`nsemclaw.provider`、`nsemclaw.model`）
- `nsemclaw.run.duration_ms`（直方图，属性：`nsemclaw.channel`、`nsemclaw.provider`、`nsemclaw.model`）
- `nsemclaw.context.tokens`（直方图，属性：`nsemclaw.context`、`nsemclaw.channel`、`nsemclaw.provider`、`nsemclaw.model`）

消息流：

- `nsemclaw.webhook.received`（计数器，属性：`nsemclaw.channel`、`nsemclaw.webhook`）
- `nsemclaw.webhook.error`（计数器，属性：`nsemclaw.channel`、`nsemclaw.webhook`）
- `nsemclaw.webhook.duration_ms`（直方图，属性：`nsemclaw.channel`、`nsemclaw.webhook`）
- `nsemclaw.message.queued`（计数器，属性：`nsemclaw.channel`、`nsemclaw.source`）
- `nsemclaw.message.processed`（计数器，属性：`nsemclaw.channel`、`nsemclaw.outcome`）
- `nsemclaw.message.duration_ms`（直方图，属性：`nsemclaw.channel`、`nsemclaw.outcome`）

队列 + 会话：

- `nsemclaw.queue.lane.enqueue`（计数器，属性：`nsemclaw.lane`）
- `nsemclaw.queue.lane.dequeue`（计数器，属性：`nsemclaw.lane`）
- `nsemclaw.queue.depth`（直方图，属性：`nsemclaw.lane` 或 `nsemclaw.channel=heartbeat`）
- `nsemclaw.queue.wait_ms`（直方图，属性：`nsemclaw.lane`）
- `nsemclaw.session.state`（计数器，属性：`nsemclaw.state`、`nsemclaw.reason`）
- `nsemclaw.session.stuck`（计数器，属性：`nsemclaw.state`）
- `nsemclaw.session.stuck_age_ms`（直方图，属性：`nsemclaw.state`）
- `nsemclaw.run.attempt`（计数器，属性：`nsemclaw.attempt`）
...
- `nsemclaw.webhook.error`
  - `nsemclaw.channel`、`nsemclaw.webhook`、`nsemclaw.chatId`、`nsemclaw.error`
- `nsemclaw.message.processed`
  - `nsemclaw.channel`、`nsemclaw.outcome`、`nsemclaw.chatId`、`nsemclaw.messageId`、`nsemclaw.sessionKey`、`nsemclaw.sessionId`、`nsemclaw.reason`
- `nsemclaw.session.stuck`
  - `nsemclaw.state`、`nsemclaw.ageMs`、`nsemclaw.queueDepth`、`nsemclaw.sessionKey`、`nsemclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `nsemclaw doctor`。
