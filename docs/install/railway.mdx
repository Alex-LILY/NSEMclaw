...
Deploy Nsemclaw on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/nsemclaw` — Control UI
...
- `NSEMCLAW_STATE_DIR=/data/.nsemclaw` (recommended)
- `NSEMCLAW_WORKSPACE_DIR=/data/workspace` (recommended)
- `NSEMCLAW_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Nsemclaw state + workspace so you can migrate to another host without losing config or memory.
