...
Deploy Nsemclaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/nsemclaw/nsemclaw)
...
repository configures everything needed to run Nsemclaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: NSEMCLAW_STATE_DIR
        value: /data/.nsemclaw
      - key: NSEMCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: NSEMCLAW_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/nsemclaw`.
...
If you use the original Nsemclaw repository, Render will not auto-deploy your Nsemclaw. To update it, run a manual Blueprint sync from the dashboard.
...
For Nsemclaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Nsemclaw host.
