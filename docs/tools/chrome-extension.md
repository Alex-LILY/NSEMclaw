---
summary: "Chrome extension: let Nsemclaw drive your existing Chrome tab"
read_when:
...
The Nsemclaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate nsemclaw-managed Chrome profile.
...
Nsemclaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Nsemclaw release (npm package) as static files. There is no separate “build” step.

After upgrading Nsemclaw:

- Re-run `nsemclaw browser extension install` to refresh the installed files under your Nsemclaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

Nsemclaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `NSEMCLAW_GATEWAY_TOKEN`)
...
- Open the tab you want Nsemclaw to control.
...
- `ON`: attached; Nsemclaw can drive that tab.
...
Debugging: `nsemclaw sandbox explain`
...
`nsemclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `nsemclaw browser extension install` first to copy the extension to a stable location under your Nsemclaw state directory.
...
- **This is not isolated** like the dedicated nsemclaw-managed profile.
