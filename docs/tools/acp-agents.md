...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let Nsemclaw run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask Nsemclaw in plain language to "run this in Codex" or "start Claude Code in a thread", Nsemclaw should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want Nsemclaw-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Nsemclaw native sub-agent runtime  |
...
- Nsemclaw binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, Nsemclaw returns a clear unsupported/unavailable message.
...
- If `agentId` is omitted, Nsemclaw uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, Nsemclaw may default to persistent behavior per runtime path
...
If no target resolves, Nsemclaw returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, Nsemclaw returns a clear unsupported-control error.
...
When Nsemclaw uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Nsemclaw `agentId` path).
...
```bash
nsemclaw plugins install @nsemclaw/acpx
nsemclaw config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
nsemclaw plugins install ./extensions/acpx
```
...
`@nsemclaw/acpx` now enforces a strict plugin-local pinning model:
...
- Nsemclaw startup stays non-blocking while acpx ensure runs.
