...
- **gateway host** → `nsemclaw` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.nsemclaw/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.nsemclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `nsemclaw approvals allowlist ...`).
- `nsemclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `nsemclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.nsemclaw/exec-approvals.json` directly.

CLI: `nsemclaw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
