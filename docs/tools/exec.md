...
- `gateway`/`node` approvals are controlled by `~/.nsemclaw/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Nsemclaw prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`nsemclaw security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `nsemclaw doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
