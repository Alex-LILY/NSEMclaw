---
summary: "Direct `nsemclaw agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `nsemclaw agent` (direct agent runs)

`nsemclaw agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `nsemclaw message --target`).
...
```bash
nsemclaw agent --to +15555550123 --message "status update"
nsemclaw agent --agent ops --message "Summarize logs"
...
nsemclaw agent --to +15555550123 --message "Summon reply" --deliver
