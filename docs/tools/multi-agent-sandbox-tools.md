...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `nsemclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `nsemclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=nsemclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${NSEMCLAW_STATE_DIR:-$HOME/.nsemclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
