 ...
 * `nsemclaw update`. Use it to verify the completion installation flow
...
\`nsemclaw update\`, \`nsemclaw doctor\`, and \`nsemclaw onboard\`.
...
async function main() {
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`nsemclaw completion --install\` later to enable.`));
    return;
  }
