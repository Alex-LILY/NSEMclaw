...
const shouldBuild = (deps) => {
  if (deps.env.NSEMCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.NSEMCLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[nsemclaw] ${message}\n`);
};

const runOpenClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["nsemclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
