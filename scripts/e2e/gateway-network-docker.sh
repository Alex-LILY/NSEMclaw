...
IMAGE_NAME="nsemclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="nsemclaw-net-e2e-$$"
GW_NAME="nsemclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "NSEMCLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "NSEMCLAW_SKIP_CHANNELS=1" \
  -e "NSEMCLAW_SKIP_GMAIL_WATCHER=1" \
  -e "NSEMCLAW_SKIP_CRON=1" \
  -e "NSEMCLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
