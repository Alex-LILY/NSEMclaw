...
function resolveStateDir(): string {
  const override = process.env.NSEMCLAW_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.NSEMCLAW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".nsemclaw");
}
...
function usage(): string {
  return (
    ...
    "  NSEMCLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  NSEMCLAW_DISCORD_SMOKE_DRIVER\n" +
    "  NSEMCLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  NSEMCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  NSEMCLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  NSEMCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  NSEMCLAW_DISCORD_SMOKE_AGENT\n" +
    "  NSEMCLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  NSEMCLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  NSEMCLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  NSEMCLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.NSEMCLAW_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.NSEMCLAW_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.NSEMCLAW_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.NSEMCLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.NSEMCLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.NSEMCLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.NSEMCLAW_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.NSEMCLAW_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.NSEMCLAW_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.NSEMCLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.NSEMCLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.NSEMCLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `nsemclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
