...
const urlRaw = getArg("--url") ?? process.env.NSEMCLAW_GATEWAY_URL;
const token = getArg("--token") ?? process.env.NSEMCLAW_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.NSEMCLAW_RUN_DANGEROUS === "1";
...
if (!urlRaw || !token) {
  ...
  console.error(
    "Usage: bun scripts/dev/ios-node-e2e.ts --url <wss://host[:port]> --token <gateway.auth.token> [--node <id|name-substring>] [--dangerous] [--json]\n" +
      "Or set env: NSEMCLAW_GATEWAY_URL / NSEMCLAW_GATEWAY_TOKEN",
  );
  process.exit(1);
}
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "nsemclaw ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "nsemclaw-dev-ios-node-e2e",
    },
    locale: "en-US",
