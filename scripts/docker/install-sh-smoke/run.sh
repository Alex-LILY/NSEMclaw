...
INSTALL_URL="${NSEMCLAW_INSTALL_URL:-https://nsemclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${NSEMCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${NSEMCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="nsemclaw"
PACKAGE_NAME="${NSEMCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (NSEMCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${NSEMCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${NSEMCLAW_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
