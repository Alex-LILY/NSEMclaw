...
# https://til.simonwillison.net/llms/nsemclaw-docker
#
# Installation:
...
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/nsemclaw"
  "${HOME}/workspace/nsemclaw"
  "${HOME}/projects/nsemclaw"
  "${HOME}/dev/nsemclaw"
  "${HOME}/code/nsemclaw"
  "${HOME}/src/nsemclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    ...
    if [[ "$response" =~ ^[Nn] ]]; then
      ...
      echo "  export CLAWDOCK_DIR=/path/to/nsemclaw"
      return 1
    fi
    ...
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/nsemclaw/nsemclaw.git ~/nsemclaw"
    echo "  cd ~/nsemclaw && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^NSEMCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d nsemclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart nsemclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f nsemclaw-gateway
}
...
clawdock-workspace() {
  cd ~/.nsemclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec nsemclaw-gateway \
    bash -c 'echo "alias nsemclaw=\"./nsemclaw.mjs\"" > /tmp/.bashrc_nsemclaw && bash --rcfile /tmp/.bashrc_nsemclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm nsemclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build nsemclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "NSEMCLAW_GATEWAY_TOKEN=$token" nsemclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" nsemclaw-gateway \
    bash -c './nsemclaw.mjs config set gateway.remote.token "$TOKEN" && ./nsemclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec nsemclaw-gateway \
    bash -c "./nsemclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart nsemclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm nsemclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec nsemclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'nsemclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (nsemclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to nsemclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.nsemclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'nsemclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'nsemclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.nsemclaw.ai${_CLR_RESET}"
  echo ""
}
